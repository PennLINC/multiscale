...
% same for MAD
LHmadVecs=zeros(length(Index_l),30);
RHmadVecs=zeros(length(Index_r),30);
...
for h=1:2;
	...
	if (h==1)
		...
		LHmadVecs(:,1)=a.VertexChange(Index_l);
	elseif (h==2)
		RHchangeVecs(:,1)=a.VertexChange(Index_r);
		RHmadVecs(:,1)=a.VertexChange(Index_r);
	end
	...
	for K=2:30
		...
		if (h==1)
			...
			LHmadVecs(:,K)=MADKstruct.VariabilityLabel_lh(Index_l);
		elseif (h==2)
			RHchangeVecs(:,K)=a.VertexChange(Index_r);
			RHmadVecs(:,K)=MADKstruct.VariabilityLabel_rh(Index_r);
		end
	end
end
...
writetable(table(LHmadVecs),'/cbica/projects/pinesParcels/results/aggregated_data/vertexWiseMAD_LH.csv');
writetable(table(RHmadVecs),'/cbica/projects/pinesParcels/results/aggregated_data/vertexWiseMAD_RH.csv');
