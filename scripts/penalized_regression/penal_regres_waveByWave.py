...
# permutation runs commented out with triple # in this version
...
# load in mega b/w file
megabwfn = '/cbica/projects/pinesParcels/results/aggregated_data/ind_vertices_bw_allscales.mat'
megabwf = h5py.File(megabwfn,'r')
megabw=megabwf['dfbw']


# gcv parameter setting (zaixu's range)
alphas = np.exp2(np.arange(16) - 10)

# for each scale
for K in range(2,31):
	print(K)	
	# Subject b/w vals for this scale (17734 x 693 at K)	
	np_data=np.array(megabw[:,K-2,:]);
	...
	# NO need to reshape so that b/w features are a 17734 vector for each subject
	if (np_data.shape[1] == (17734)):
		print ("Vectorized b/w vals seem to be the correct length")
	else:
		print("Vectorized b/w vals are not 17734 long, back to the drawing board bub")
	
	# slap age on to the end of feature vectors for prediction dataframe format
	masterdf=np.hstack((np_data,merged))
	...
	varofintnum=np_data.shape[1]+1
	
	# Divide to predict var of int columns
	topogvecs=masterdf[:,0:np_data.shape[1]]
	...
	# average feature weights = number of verts (same for all scales)
	all_featureWeights=np.empty([12,(17734)])
	...
	featureweightsFN='/cbica/projects/pinesParcels/data/aggregated_data/BW_FeatureWeightsScale_' + str(K) + '.csv'
...
#### Sep. loop with same setup for vertexwise multiscale coefficients
# load in slopes
slopesfn='/cbica/projects/pinesParcels/results/EffectVecs/rSlopes.mat'
slopesf = sio.loadmat(slopesfn)
slopes=slopesf['rSlope']
# Because this uses R output, they are in the same order as the variables of int loaded in prior to their pandas rearrangement
merged_R_order=np.hstack((slopes,df))
# No need to loop, just isolate the var of int for preds (age)
varofintnum=slopes.shape[1]+1	
# and the topography(ish) vertex values
topogvecs=merged_R_order[:,0:slopes.shape[1]]
varofint=masterdf[:,varofintnum]
all_preds=np.empty([12,1])
all_preds_alphas=np.empty([12,1])
all_featureWeights=np.empty([12,(17734)])
# we ARE looping over 12 runs though, for more stability
for split in range(0,12):
	xtrain,xtest,ytrain,ytest=train_test_split(topogvecs,varofint,test_size=0.33,random_state=(split))
	r2_vec_split=[]
	lm = sklearn.linear_model.RidgeCV(alphas=alphas, store_cv_values=True).fit(xtrain,ytrain)
