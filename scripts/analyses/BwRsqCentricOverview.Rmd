...
for (i in 1:29){
  ...
  ageTogbwDR2[i]<-DeltaR2EstVec(loopdf$Gbw)
...
dataforplot<-data.frame(ageTogbw,ageTogbwDR2,gbwToEF,SigMed,seq(2, 30))
  ...
  # one weird trick to get binarized transmodality class vector for same scale (Doctors hate him!)
   ...
   #     print('You done goofed, internet police are on their way')
    ...
    # get average bw network connectivty age correlation for this network at this scale
      ...
      # this is the step that puts DR2 in place of what was previously age cor.
...
ggplot(MediationDF,aes(tmvec,AB_Est,color=domnetvec,alpha=netpropvec^2)) + scale_color_manual(values=c('#007500','#c1253c','#d77d00','#b8cf86','#3281ab','#b61ad0','#670068')) + xlab("Transmodality") + ylab('AB Path Coef.') +theme_classic(base_size = 40) + geom_point(size=7,aes(tmvec,AB_Est)) + ggtitle('AB Path Mediation Coef by Transmodality')+guides(alpha=FALSE,color=guide_legend(title="Maximal Y7 Overlap"))+theme(legend.position="top")
...
# adding this in to confidently map global-bw DR2 to the edges dataframe
GbwDR2vec=rep(0,length(colnames(individ_scalebybw_df)))
...
for (i in 1:length(colnames(individ_scalebybw_df))){
    ...
    scalevec[i]=scale
    # adding this in to confidently map global-bw DR2 to the edges dataframe
    GbwDR2vec[i]=ageTogbwDR2[scale-1]
...
## merged vectors
BwAgeCorTMDifDf<-data.frame(tmdifvec,ageEfvec,noAge_EFEfVec,EFEfvec,scalevec,Net1Vec,Net2Vec,Net1Vec17,Net2Vec17,fdr_ageEfPvec,fdr_EFEfPvec,fdr_noAge_EFEfPVec,ageDR2vec,occdifvec,all3pgDifvec,domnetSig1,domnetSig2,domnetAgelessSig1,domnetAgelessSig2,domnetSigAge1,domnetSigAge2,net1tmvec,net2tmvec,GbwDR2vec)
# fdr age dr2
...
```
```{r}
# set seed because this might not be 100% deterministic
set.seed(1)
# gmms to pull out edge classes
library(mixtools)
#' @param x Input data
#' @param mu Mean of component
#' @param sigma Standard deviation of compon
plot_mix_comps <- function(x, mu, sigma, lam) {
  lam * dnorm(x, mu, sigma)
}

set.seed(1)
wait <- BwAgeCorTMDifDf$tmdifvec
mixmdl <- normalmixEM(wait, k = 3)

data.frame(x = BwAgeCorTMDifDf$tmdifvec) %>%
  ggplot() +
...
# great, looks like about 1/3rd are short TM distance connections and 2/3rds are longer
# now pull same # of edges from full distribution, short-range distrib., and long-range distrib.

Pull_n_edges <- function(n) {
  # initialize short, long, and all range indices
  SRI=NULL
  LRI=NULL
  ARI=NULL
  # initialize copy of transmodality difference vector for manipulating
  loop_tmdifvector<-BwAgeCorTMDifDf$tmdifvec
  # calculated classes are gaussian distrib, use rnorm to match that
  sdsInSTDunits<-rnorm(n)
  SRUnits<-sdsInSTDunits*mixmdl$sigma[1]+mixmdl$mu[1]
  LRUnits<-sdsInSTDunits*mixmdl$sigma[3]+mixmdl$mu[3]
  # loop over list of transmodality differences n times to sample WITHOUT replacement
  for (i in 1:n){
  # pull short range edge indices that are best match to rnorm generated SDs
  SRI[i]<-which(abs(SRUnits[i]-loop_tmdifvector)==min(abs(SRUnits[i]-loop_tmdifvector)))
  # take value out
  loop_tmdifvector[SRI[i]]<- -999
  # pull long range edge indices that are best match to rnorm generated SDs
  LRI[i]<-which(abs(LRUnits[i]-loop_tmdifvector)==min(abs(LRUnits[i]-loop_tmdifvector)))
  # take value out
  loop_tmdifvector[LRI[i]]<- -999
  }
  # take half of each for "full gradient sampling" comparison approach
  ARI[1:(n/2)]=SRI[1:(n/2)]
  ARI[(n/2):n]=LRI[(n/2):n]
  edgesSampled=data.frame(SRI,LRI,ARI)
  return(edgesSampled)
}

# trial runs with 30 more features in each iteration
edgeNumbers<-seq(30,1000,by=30)
parentFilePath<-'/cbica/projects/pinesParcels/results/EffectVecs/'
# write em out for penalregress, age version
for (i in 1:length(edgeNumbers)){
  # sample edges from above function
  edgesDF<-Pull_n_edges(edgeNumbers[i])
  # write out short range edges
  SRE_forRRfc<-cbind(masterdf$Age,masterdf$Motion,masterdf[,edgesDF$SRI])
  SRfn<-paste(parentFilePath,'SR',edgeNumbers[i],'_for_bw_RRfc.csv',sep='')
  write.table(SRE_forRRfc,SRfn,row.names = F,col.names = F,sep = ',')
  # write out long range
  LRE_forRRfc<-cbind(masterdf$Age,masterdf$Motion,masterdf[,edgesDF$LRI])
  LRfn<-paste(parentFilePath,'LR',edgeNumbers[i],'_for_bw_RRfc.csv',sep='')
  write.table(LRE_forRRfc,LRfn,row.names = F,col.names = F,sep = ',')
  # write out all range
  ARE_forRRfc<-cbind(masterdf$Age,masterdf$Motion,masterdf[,edgesDF$ARI])
  ARfn<-paste(parentFilePath,'AR',edgeNumbers[i],'_for_bw_RRfc.csv',sep='')
  write.table(ARE_forRRfc,ARfn,row.names = F,col.names = F,sep = ',')
}
```
...
ggplot(data = BwAgeCorTMDifDf, aes(x = factor(scalevec), y = tmdifvec,color=ageEfvec)) +geom_violin(size=1,alpha=.5,draw_quantiles =c(.33,.66))+ylab('Gradient Distance')+xlab("# of Communities") +theme_classic(base_size=28)+ scale_colour_gradientn(colours=c('#0b0bff','#8585ff','gray90','#ff9191','#ff0a0a'),values = c(0,0.3,0.5,0.7,1),breaks=c(-.3,0,.3), labels = c(-.3,0,.3),limits=c(-.35,0.35),name=expression(paste(Delta,R^2[adj])))

#### overlay gbw ~ Age DR2

# knabbin this vector from one of the earlier chunks
BwAgeCorTMDifDf$GbwDr2<-GbwDR2vec

ggplot(data = BwAgeCorTMDifDf, aes(x = factor(scalevec), y = tmdifvec)) +geom_violin(size=1,alpha=.3,draw_quantiles =c(.5),bw=.5)+geom_line(data=BwAgeCorTMDifDf,aes(x=factor(scalevec),y=GbwDr2,color=GbwDr2))+ylab('Transmodality Difference')+xlab("# of Communities") +theme_classic(base_size=28)+ scale_colour_gradientn(colours=c('#0b0bff','#8585ff','gray90','#ff9191','#ff0a0a'),values = c(0,0.3,0.5,0.7,1),breaks=c(-.3,0,.3), labels = c(-.3,0,.3),limits=c(-.35,0.35),name=expression(paste(Delta,R^2[adj])))


ggplot(data = BwAgeCorTMDifDf) +geom_violin(aes(x = factor(scalevec), y = (tmdifvec*.04)),size=1,alpha=.3,draw_quantiles =c(.5),bw=.5)+geom_point(data=BwAgeCorTMDifDf,aes(x=scalevec,y=GbwDr2,color=GbwDr2))+ylab('Transmodality Difference')+xlab("# of Communities") +theme_classic(base_size=28)+ scale_colour_gradientn(colours=c('#0b0bff','#8585ff','gray90','#ff9191','#ff0a0a'),values = c(0,0.3,0.5,0.7,1),breaks=c(-.3,0,.3), labels = c(-.3,0,.3),limits=c(-.1,0.1),name=expression(paste(Delta,R^2[adj])))


ggplot(data = BwAgeCorTMDifDf) +geom_point(aes(x=as.factor(scalevec),y=(tmdifvec*-.03)+.2),alpha=.5,shape='-',size=10,color='black')+geom_point(data=BwAgeCorTMDifDf,aes(x=scalevec-1,y=GbwDr2,color=GbwDr2),size=15)+ylab('Transmodality Difference')+xlab("# of Communities") +theme_classic(base_size=28)+ scale_colour_gradientn(colours=c('#0b0bff','#8585ff','gray90','#ff9191','#ff0a0a'),values = c(0,0.3,0.5,0.7,1),breaks=c(-.3,0,.3), labels = c(-.3,0,.3),limits=c(-.13,0.13),name=expression(paste(Delta,R^2[adj])))+geom_violin(aes(x = factor(scalevec), y = (tmdifvec*-.03)+.2),size=1,alpha=.3,bw=.01)
```
