...
# for a few different train and test splits
	...
	all_preds_alphas[split,1]=alpha_AD
	all_preds_alphas[split,2]=alpha_AI
	#alpha_p=lm_p.alpha_
	...
	all_preds[split,1]=pred_obs_r2_AD
	all_preds[split,2]=pred_obs_r2_AI
	#all_permut_preds[split,:]=pred_obs_r2_permut	
	
# mean age predictions
mean_preds_AD=np.average(all_preds[:,1])
mean_preds_AI=np.average(all_preds[:,2])
#mean_preds_permut=np.average(all_permut_preds[:])
# mean alphas
mean_alphas_AD=np.average(all_preds_alphas[:,1])
mean_alphas_AI=np.average(all_preds_alphas[:,2])
...
summary_preds[2,1]=mean_alphas_AI
summary_preds[1,0]=mean_preds_AD
summary_preds[1,1]=mean_alphas_AD
summary_preds[2,0]=mean_preds_AI
