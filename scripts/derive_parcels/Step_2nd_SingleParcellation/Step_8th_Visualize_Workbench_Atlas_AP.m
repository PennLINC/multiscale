...
%commented clear out for K
%clear

Folder = '/cbica/projects/pinesParcels/data/SingleParcellation/SingleAtlas_Analysis';
VisualizeFolder = [Folder '/Atlas_Visualize'];
mkdir(VisualizeFolder);

%% Group probability atlas
GroupAtlasLoading_Mat = load([Folder '/Group_AtlasLoading_' num2str(K) '.mat']);
for i = 1:K
  % left hemi
  V_lh = gifti;
  V_lh.cdata = GroupAtlasLoading_Mat.sbj_AtlasLoading_lh(i, :)';
  V_lh_File = [VisualizeFolder '/Group_lh_kK' num2str(K) 'Kk_Network_' num2str(i) '.func.gii'];
  save(V_lh, V_lh_File);
  % right hemi
  V_rh = gifti;
...
GroupAtlasLabel_Mat = load([Folder '/Group_AtlasLabel_' num2str(K) '.mat']);
ColorInfo_Atlas = [VisualizeFolder '/name_Atlas.txt'];
system(['rm -rf ' ColorInfo_Atlas]);

% create random color scheme for K with uncreative names
...
V_lh_File = [VisualizeFolder '/Group_lh_kK' num2str(K) 'Kk.func.gii'];
save(V_lh, V_lh_File);
pause(1);
V_lh_Label_File = [VisualizeFolder '/Group_lh_kK' num2str(K) 'KkAtlasLabel.label.gii'];
...
V_rh_File = [VisualizeFolder '/Group_rh_kK' num2str(K) 'Kk.func.gii'];
save(V_rh, V_rh_File);
pause(1);
V_rh_Label_File = [VisualizeFolder '/Group_rh_' num2str(K)  'KkAtlasLabel.label.gii'];
