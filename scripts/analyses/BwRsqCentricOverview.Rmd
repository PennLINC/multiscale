...
# 12 for months
gamPredictMeAt8$Age<-(8.166667*12)
...
for (i in 1:(length(bwAvgCondf)-4)){
  ...
  # changed to get intercept at 8 years old
  InterceptVector[i]<-predict.gam(igam,gamPredictMeAt8)
...
# START vector for edge level intercept at 8 HERE
...
# appears to only work proeprly when modeled at te
g2 <- gam(ageDR2vec ~ te(net2tmvec,net1tmvec),data = doubleBwAgeCorTMDifDf)
gg_tensor(g2)+theme_classic(base_size = 40) +scale_fill_gradient2(low = "blue",high = "red",mid = "white",midpoint = 0,name=expression(paste('Age Effect(',Delta,R^2[adj],')')),guide = guide_colorbar(title.position="top",ticks.colour = "gray",
                                ticks.linewidth = 5) )+theme(legend.text=element_text(size=20),legend.key.height =unit(2.3,"cm"),legend.title = element_text(size=25))+ggtitle("")+guides(alpha=FALSE)+xlab("Community Transmodality")+ylab("Community Transmodality")


# bart method
b<-getViz(g2)
plot(sm(b, 1))+ l_fitRaster() + l_fitContour() +scale_fill_gradient2(low = "blue",high = "red",mid = "white",midpoint = 0)+labs(fill = expression(paste('Age Effect(',Delta,R^2[adj],')')))+ggtitle("")+xlab("Community Transmodality")+ylab("Community Transmodality")
...
# figure 3 flag
Bwspan<-ggplot(LongAgeSpan_plotdf2[derivSigWholeSpline,],aes(agespans,tmvecJit,color=derivs,group=CIgroupingInd,alpha=AlphaSig)) +geom_line(size=3) +labs(x = 'Age', y = 'Transmodality') +theme_classic(base_size = 40)+ xlim(c(8,23))+ scale_colour_gradientn(colours=c('#0b0bff','#8585ff','white','#ff9191','#ff0a0a'),values = c(0,.4,.5,.6,1),breaks=breaks, labels = breaks,limits=c(-.0031,0.0031),name="Change Per Year (B/w Con.)")+theme(legend.text=element_text(size=30),legend.title = element_text(size=30),legend.position="top",legend.key.width = unit(3, "cm"))+ylim(c(min(tmvecJit),max(tmvecJit)))+guides(alpha=FALSE,color = guide_colorbar(title.position="top",ticks.colour = "gray",
...
# figure 4 flag
ggplot(BwAgeCorTMDifDf[BwAgeCorTMDifDf$ageDR2Pvec<0.05,],aes(x=tmdifvec,y=ageDR2vec))+ geom_text(size=10,label="\u25D6",family="Arial Unicode MS",aes(color=Net1Vec))+scale_color_manual(values=ycolors,limits=Y7vec)+geom_text(aes(x=tmdifvec,y=ageDR2vec,color=Net2Vec),size=10,label="\u25D7",family="Arial Unicode MS")+theme(legend.position = "none") + xlab('Transmodality Difference') + ylab(expression(paste('Age Effect(',Delta,R^2[adj],')')))+theme_classic(base_size = 40)+geom_point(data=BwAgeCorTMDifDf[BwAgeCorTMDifDf$ageDR2Pvec>0.05,],aes(x=tmdifvec,y=ageDR2vec,fill=NULL),pch=21,color='grey',size=3)+theme(legend.position = "none")+geom_smooth(size=3,method='lm',color='black',formula=y~x)
...
# figure 5 flag
ggplot(subset(BwAgeCorTMDifDf, Net1Vec == 'DA'),aes(y=tmdifvec,x=scalevec))+geom_point()+geom_smooth()
```
