...
df=np.loadtxt('/cbica/projects/pinesParcels/results/EffectVecs/forML_EF.csv',delimiter=',')
df_permut=np.loadtxt('/cbica/projects/pinesParcels/results/EffectVecs/forML_EF_permut.csv',delimiter=',')
...
pd_df=pd.DataFrame(df, columns=['bblid', 'EF' , 'Age', 'Motion', 'Sex'])
pd_df_permut=pd.DataFrame(df_permut, columns=['bblid', 'EF', 'Age', 'Motion', 'Sex'])
...
for K in range(2,31):
	...
	# for a few different train and test splits
		...
		xtrain,xtest,ytrain,ytest=train_test_split(topogvecs,varofint,test_size=0.33,random_state=(split+10))
	...
	featureweightsFN='/cbica/projects/pinesParcels/data/aggregated_data/FeatureWeightsEF_Scale_' + str(K) + '.csv'
