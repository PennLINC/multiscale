...
# best fit so far
model=glmer(value~Age+Motion+Sex+(1 + log(Scale)|bblid),data=mdf_MotVert)
...
model=glmer(value~Age+Motion+Sex+log(Scale)+(1 + log(Scale)|bblid),data=mdf_globalVerts)
###v<-visreg(model,"Scale",by="bblid",re.form=~(1 + log(Scale)|bblid),plot=F)
```

```{r}
# pretty computationally intensive chunk to run locally - done so to deal with cubic pushing against installation of "melt"
library(reshape2)
library(lme4)
demo<-read.csv('/cbica/projects/pinesParcels/data/pnc_demo.csv')
ageSex<-data.frame(demo$ageAtScan1,as.factor(demo$sex),demo$scanid,demo$bblid)
