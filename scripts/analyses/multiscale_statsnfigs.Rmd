...
tc<-ggplot(data=mdata,aes(x=variable,y=value,group=bblid,color=Age)) +geom_line(alpha = 0.25,size=1)+ scale_color_gradientn(colors=c("yellow","purple")) + theme_minimal()+labs(title="Total cost over scales")
...
mcrecnerrdf<-data.frame(mreconerr,community_vec,SelectedScales)
ggplot(data=mcrecnerrdf,aes(y=mreconerr,x=community_vec))+geom_step(col='black',size=2)+theme_minimal(base_size = 28)+ scale_x_continuous(name ="# of Communitites",  breaks=seq(2, 30, 4))+ylab("Reconstruction Error (a.u.)")
...
changevec<-scalevec
changevecVar<-scalevec
for (i in 2:29){
  changevec[i]<-mcrecnerrdf$mreconerr[i]-mcrecnerrdf$mreconerr[i-1]
  changevecVar[i]<-varoverscales$SoftVar[i]-varoverscales$SoftVar[i-1]
}
...
varoverscales$change<-changevecVar
mcrecnerrdf$SoftVar<-varoverscales$SoftVar
mcrecnerrdfchangeVar<-changevecVar
...
# dual plot of recon error and variab.
avgrecon_softParVar<-ggplot(data=mcrecnerrdf,aes(y=mreconerr,x=community_vec))+geom_step(size=3,color='#262228') +geom_step(aes(y=SoftVar*45e5,x=community_vec),size=3,color='#0375B4',alpha=.7) + scale_x_continuous(name ="# of Communitites",  breaks=seq(2,30,4))+scale_y_continuous(sec.axis=sec_axis(trans =as.formula(~./45e5),name = 'Variability (MAD)'))+labs(y='Reconstruction Error (a.u.)')+theme_minimal(base_size = 25)+ theme(axis.title.y.left =  element_text(color = '#262228'),axis.text.y.left =  element_text(color = '#262228'),axis.title.y.right = element_text(color = '#0375B4'), axis.text.y.right = element_text(color = '#0375B4'))#+ylim(108000,208000)
...
ggarrange(avgrecon_softParVar,avgSPvar,nrow = 1)
```
...
for (i in 1:29){
  ...
  print(ggplot(indglobWin_motSexC,aes(Age,indglobWin_motSexC[,i+4])) + geom_point() + geom_smooth() +ylab(paste("Ind Global Win at Scale", i+1)))
    print(ggplot(indglobBw_motSexC,aes(Age,indglobBw_motSexC[,i+4])) + geom_point(color='#AAAAAA',alpha=.5) + geom_smooth(method='gam',formula=y~s(x,k=3),color='black') +ylab(paste("B/w at Scale", i+1))+theme_classic(base_size=30))
...
indagecor<-correlations_over_scales(ind_segcors,'Segregation-Age Correlation')
...
youngindagecor<-correlations_over_scalesplot(ind_segcors,'Segregation-Age Correlation: 8-13 Years')
...
midindagecor<-correlations_over_scalesplot(ind_segcors,'Segregation-Age Correlation: 13-18 Years')
...
oldindagecor<-correlations_over_scalesplot(ind_segcors,'Segregation-Age Correlation: 18-23 Years')
...
SegregR2VecEst<-as.numeric(lapply(masterdf[,indiv_nsegcols_ind],DeltaR2EstVec))
SegregPVecEst<-as.numeric(lapply(masterdf[,indiv_nsegcols_ind],DeltaPEstVec))

segdf<-data.frame(tmvec,scalesvec,domnetvec,netpropvec,SegregR2VecEst,SegregPVecEst)

####

#fdr it
SegregPFDR<-p.adjust(SegregPVecEst,method='fdr')
segdf$FDRed<-SegregPFDR

seg_age_tm<-ggplot(segdf,aes(tmvec,SegregR2VecEst,color=domnetvec,alpha=netpropvec^2)) + geom_point(size=6)+ scale_color_manual(values=c('#007500','#c1253c','#d77d00','#b8cf86','#3281ab','#b61ad0','#670068')) + xlab("Transmodality") + ylab(expression(paste(Delta,R^2[adj]))) +theme_classic(base_size = 28) + ggtitle('Segregation Variance Captured by Age') +guides(alpha=FALSE,color=guide_legend(title="Maximal Y7 Overlap"))+theme(legend.position="top")
```
...
for (i in 1:length(masterdf[,indiv_nsegcols_ind])){
  ...
  # jk, no centering in this version
...
withinBetweenClassConVec<-vector()
...
for (i in 1:2){
  ...
  # one weird trick to get binarized transmodality class vector for same scale (Doctors hate him!)
   ...
   #     print('You done goofed, internet police are on their way')
    ...
    print(paste('# of unimodal to transmodal community ratio:', unilength/translength))
    
    # use triangular numbers to derive number of cross-modal and within-modal connections at this scale
    unimodal2unimodalCons=((unilength-1)*unilength)/2
    transmodal2transmodalCons=((translength-1)*translength)/2
    # cross modal as the remaining b/w cons
    crossmodal=(((K-1)*K)/2)-unimodal2unimodalCons-transmodal2transmodalCons
    withinmodal=unimodal2unimodalCons+transmodal2transmodalCons
    print(paste('# of within-tm class connections vs between tm-class connections:', withinmodal/crossmodal,K,sep='|'))
    withinBetweenClassConVec[K]<-withinmodal/crossmodal
  }
...
ggplot(BwAgeCorTMDifDf,aes(x=tmdifvec,y=ageEfvec)) + geom_point(alpha=.07,size=5,shape="\u25d6") + xlab('Transmodality Difference') + ylab('Age Partial Correlation') + ggtitle('Pairwise Between-Network Age-FC Relations')
...
### scratch paper sections ###
### indices ofsegregators from integrators
#BwAgeCorTMDifDf_segregs<-which(BwAgeCorTMDifDf$ageEfvec < 0)
#BwAgeCorTMDifDf_integrs<-which(BwAgeCorTMDifDf$ageEfvec > 0)
#BwAgeCorTMDifDf_EF<-data.frame(tmdifvec,noAge_EFEfVec)
#segregdf<-BwAgeCorTMDifDf_EF[BwAgeCorTMDifDf_segregs,]
#integrgdf<-BwAgeCorTMDifDf_EF[BwAgeCorTMDifDf_integrs,]
#a=gam(noAge_EFEfVec~s(tmdifvec,k=4),data=segregdf)
#b=gam(noAge_EFEfVec~s(tmdifvec,k=4),data=integrgdf)
### end of scratch paper section
...
indglobbw<-data.frame(cbind(masteref$bblid,masteref$Age/12,masteref$Sex,masteref$Motion,masteref$F1_Exec_Comp_Cog_Accuracy,gbw))
...
indEFcor<-correlations_over_scales(ind_bwcors,"Global B/w-EF Correlation - Individ. Partitions")
...
#ggplot(ind_GBwcors,aes(x=ind_GBwcors$X1,y=ind_GBwcors$X2,col='blue'))+geom_line()+geom_line(aes(ind_GBwcors$X1,ind_GBwcors$X3,col='red'))+xlab('Scale')+ylab('Value')+scale_color_hue(labels=c('WithinClass:Cross Class B/w connections','B/w x Age Correlation'))
```
