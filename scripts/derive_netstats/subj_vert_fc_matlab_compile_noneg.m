function subj_vert_fc(fc_configfp)
	...
	% literally the only different line between regular run and non-negative run is here: threshold out values below 0
	negIndices=find(ba_conmat<0);
	ba_conmat(negIndices)=0;
	% for each scale
	for K=2:max(Krange)
		tic
		K	
