...
# get permutation pred. vs. obs. correlations in 
...
```{r}
ggplot(plotdf,aes(x=AgeMotRegrEF,y=predEF_AI)) + scale_fill_gradientn(colors=plasma_pal)+geom_point(size=2,alpha=.6)+geom_smooth(method='lm',color='gray',size=2)+theme_classic(base_size=25) + xlab("Observed") + ylab("Predicted")+ggtitle('Executive Function')
```
