...
coarseKref<-read.csv('Y:/results/aggregated_data/fc/network_k2Correspondence_overscales.csv',stringsAsFactors = F)
...
# plot distribution of k=2 alignment by color (unimodal=purple, transmodal=yellow)
# x = functional hierarchy, y = scale
coarseplotdf<-data.frame(tmvec,domnetvec2,scalesvec2,netpropvec2)
plot<-ggplot(coarseplotdf[3:464,],aes(tmvec,scalesvec2)) + geom_point(size=3,aes(color=domnetvec2,alpha=netpropvec2))+ scale_color_manual(values=c('red','blue')) + xlab("Functional Hierarchy") + ylab('Scale') +theme_classic(base_size = 40) +guides(color=guide_legend(title="K=2 Overlap"),alpha=guide_legend(title="% vertices in K=2 label"))+theme(legend.title=element_text(size=25),legend.text=element_text(size=25))
library(ggExtra)
ggMarginal(plot,type='histogram',margins = 'x')
```
...
bootstrapRDS<-readRDS('Y:/data/Age_NetLevel_bootInfo.rds')
...
```{r}
# primaries dist
pdist=read.csv('Y:/results/aggregated_data/NetworkPrimariesDist.csv')
...
bwdfPD<-bwdf
bwdfPD$CSulcDist<-pdist$nwise_eucs_1
bwdfPD$CalcDist<-pdist$nwise_eucs_2
bwdfPD$ttDist<-pdist$nwise_eucs_3


ggplot(bwdfPD,aes(CSulcDist,CalcDist)) + geom_point(size=6,alpha=.8,aes(color=tmvec)) + xlab("C sulc Dist") + ylab('Calc Dist') +theme_classic(base_size = 40) +guides(color=guide_legend(title="Order"))+theme(legend.title=element_text(size=10),legend.text=element_text(size=10))


plot3d(bwdfPD$CSulcDist,bwdfPD$CalcDist,bwdfPD$ttDist,col=bwdfPD$tmvec+10)
scatter3D(bwdfPD$CSulcDist,bwdfPD$CalcDist,bwdfPD$ttDist,colvar = as.numeric(bwdfPD$tmvec))
```
