...
avgrecon<-ggplot(data=mcrecnerrdf,aes(y=mreconerr,x=community_vec))+geom_line(col='black',size=2)+theme_minimal(base_size = 28)+geom_step(data=mcrecnerrdf,aes(y=mreconerr,x=community_vec))+geom_step(data=mcrecnerrdf[2:29,],aes(y=abs(change*10),x=community_vec))+ggtitle("Reconstruction Error Over Scales")+ scale_x_continuous(name ="# of Communitites",  breaks=seq(2, 30, 4))+ylab("Recon. Error")
avgSPvar<-ggplot(data=varoverscales,aes(y=SoftVar,x=community_vec))+geom_line(col='black',size=2)+theme_minimal(base_size = 28)+ggtitle("Loading Variability (MAD)")+ scale_x_continuous(name ="# of Communitites",  breaks=seq(2, 30, 4))+ylab("Avg. MAD")
avgHPvar<-ggplot(data=varoverscales,aes(y=HardVar,x=community_vec))+geom_line(col='black',size=2)+theme_minimal(base_size = 28)+ggtitle("Label Variability (Entropy)")+ scale_x_continuous(name ="# of Communitites",  breaks=seq(2, 30, 4))+ylab("Avg. Entropy")
...
ggplot(LongAgeSpan_plotdf2,aes(agespans,tmvec,color=derivs,group=CIgroupingInd)) +geom_line(size=5,alpha=.3) +labs(title='Network Segregation Effect Manifestation', x = 'Age', y = 'Transmodality') +theme_classic(base_size = 28)+ xlim(c(8,23))+ scale_colour_gradientn(colours=c(black,'blue',black,'red','yellow'),values = c(0,.215,.265,.315,1),name="Change Per Year")
...
# and for negative PC
f2=read.csv('/cbica/projects/pinesParcels/results/aggregated_data/ScaleAgeWin_TM_interaction.csv')
ggplot(f2,aes(x=CMTM,y=Var2)) + geom_pointdensity(adjust=.5) + scale_color_viridis_c() +xlab('Transmodality') + ylab('Change in Age-Between Relation (Slope) over Scales')
