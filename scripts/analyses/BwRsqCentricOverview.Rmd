...
DeltaR2EstVec_ageSubset<-function(df,x){
  
  # relevant df
  scaledf<-df[,c(2,3,4,x)]
  colnames(scaledf)<-c('Age','Sex','Motion','varofint')
...
for (i in 1:2){
  ...
  # one weird trick to get binarized transmodality class vector for same scale (Doctors hate him!)
    ...
    for (N in 1:K){
      ...
      # start from K index
      Nind<-Kind[N]
      bwAgeCorVecs[Nind,5]=K
      bwAgeCorVecs[Nind,6]=N
...
# save em out to cubic for viz of delta r^2 by community
forBwAgeDR2viz<-cbind(bwAgeCorVecs$avg_bw,bwAgeCorVecs$K,bwAgeCorVecs$N)
write.table(forBwAgeDR2viz,'/cbica/projects/pinesParcels/results/aggregated_data/fc/bwAge_dr2_overscales_forviz.csv',sep=",",col.names=F,row.names = F)
```
...
nonthresh<-ggplot(BwAgeCorTMDifDf,aes(x=tmdifvec,y=ageEfvec)) + geom_point(alpha=.07,size=5) + xlab('Transmodality Difference') + ylab('Age Partial Correlation') + ggtitle('Pairwise Between-Network Age-FC Relations')
...
TMdifDR2<-ggplot(BwAgeCorTMDifDf,aes(x=tmdifvec,y=ageDR2vec)) + geom_point(alpha=.07,size=5) + xlab('Transmodality Difference') + ylab(expression(paste(Delta,R^2[adj]))) + ggtitle('Age-FC Relations')+theme_classic(base_size=35)
...
# adapt four color scheme to b/w-con-level feautres
granularFourscalesscheme<-vector(length=4495)
for (i in 1:length(granularFourscalesscheme)){
  scaleOfInt<-scalevec[i]
  # +1 because fourscale scheme is 1:29 and scales are 2:30
  colorOfInt<-fourscalescheme[scaleOfInt-1]
  granularFourscalesscheme[i]<-colorOfInt
}

# adapt to weird factor ordering
# color vector for highlighting 4 different scales
grayHexa<-'#AAAAAA'
Weirdfourscalescheme<-rep(grayHexa,29)
# scale 4 - purple
Weirdfourscalescheme[24]<-'#6600CC'
# scale 7 - yellow
Weirdfourscalescheme[27]<- '#FFCC00'
# scale 13 - black
Weirdfourscalescheme[4]<- '#000000' 
# scale 20 - red
Weirdfourscalescheme[13]<- '#CC0000'
# big violin plot
ggplot(data = BwAgeCorTMDifDf, aes(x = as.factor(scalevec), y = tmdifvec,color=as.factor(scalevec))) +
  geom_violin(alpha = .8) + scale_color_manual(values=Weirdfourscalescheme)+
  geom_boxplot(width=0.3)+
  geom_point(size = 2, alpha = 0.8) +
  raincloud_theme+theme_classic(base_size=40)+ylab('Modal Dissimilarity')+theme(legend.position = 'none')+xlab('Scale')
```
...
agM_y7<-ggplot(ageMitigateddf,aes(x=tmdifvec,y=EFEfvec,color=Net1Vec)) + geom_text(size=10,label="\u25D6",family="Arial Unicode MS") + xlab('Transmodality Difference') + ylab('EF Partial Correlation') + ggtitle('Pairwise Between-Network Age-mitigated EF-FC Relations')+scale_color_manual(values=ycolors,limits=Y7vec)+geom_text(aes(x=tmdifvec,y=EFEfvec,color=Net2Vec),size=10,label="\u25D7",family="Arial Unicode MS")+theme_classic()+xlim(0,12)+ylim(-.3,.3)+geom_smooth(method='lm', formula= y~x,color='gray82',size=2)
...
### bottom section is likely more pragmatic: hard to estimate and regress stable age effect in thousands of FC features with GAM, but relatively straightforward to assess age effect in EF with GAM once and use that as outcome variable

# write out motion-regressed version 
MotRegrEF<-gam(F1_Exec_Comp_Cog_Accuracy~Motion,data=masteref)$residuals
# write out age and motion-regressed version
AgeMotRegrEF<-gam(F1_Exec_Comp_Cog_Accuracy~s(Age,k=4)+Motion,data=masteref)$residuals
# pure age prediction version for comparison
MotRegrAge<-lm(Age~Motion,data=masteref)$residuals

AgeDepEF<-cbind(masterdf[,indiv_bwcols_ind],MotRegrEF)
AgeIndepEF<-cbind(masterdf[,indiv_bwcols_ind],AgeMotRegrEF)
AgeOnly<-cbind(masterdf[,indiv_bwcols_ind],MotRegrAge)
...
write.table(AgeOnly,'/cbica/projects/pinesParcels/results/EffectVecs/AgeOnly',sep=',', col.names = F,quote = F,row.names=F)
```
...
predEF_Acsv<-read.csv('/cbica/projects/pinesParcels/data/aggregated_data/SubjPreds_A.csv',header=F)
predEF_ALcsv<-read.csv('/cbica/projects/pinesParcels/data/aggregated_data/SubjPreds_AL.csv',header=F)
...
predEF_A<-predEF_Acsv[,1]/predEF_Acsv[,2]
predEF_AL<-predEF_ALcsv[,1]/predEF_ALcsv[,2]
...
#lasso
plot(masteref$Age,predEF_ADL)
cor.test(masteref$Age,predEF_ADL,method='spearman')
plot(masteref$Age,predEF_AIL)
cor.test(masteref$Age,predEF_AIL,method='spearman')
...
#lasso
plot(MotRegrEF,predEF_ADL)
cor.test(MotRegrEF,predEF_ADL,method='spearman')
plot(AgeMotRegrEF,predEF_AIL)
cor.test(AgeMotRegrEF,predEF_AIL,method='spearman')

# read in feature weights
featWeightsAD<-read.csv('/cbica/projects/pinesParcels/data/aggregated_data/FeatureWeights_AD.csv',header=F)
featWeightsAI<-read.csv('/cbica/projects/pinesParcels/data/aggregated_data/FeatureWeights_AI.csv',header=F)
featWeightsADL<-read.csv('/cbica/projects/pinesParcels/data/aggregated_data/FeatureWeights_ADL.csv',header=F)
featWeightsAIL<-read.csv('/cbica/projects/pinesParcels/data/aggregated_data/FeatureWeights_AIL.csv',header=F)
featWeightsA<-read.csv('/cbica/projects/pinesParcels/data/aggregated_data/FeatureWeights_A.csv',header=F)
featWeightsAL<-read.csv('/cbica/projects/pinesParcels/data/aggregated_data/FeatureWeights_AL.csv',header=F)

# Use feature weight vectors to show kinds of networks that are weighted
BwAgeCorTMDifDf_prWeights<-data.frame(tmdifvec,ageEfvec,noAge_EFEfVec,EFEfvec,scalevec,Net1Vec,Net2Vec,Net1Vec17,Net2Vec17,fdr_ageEfPvec,fdr_EFEfPvec,fdr_noAge_EFEfPVec,as.numeric(featWeightsAD$V1),as.numeric(featWeightsAI$V1),as.numeric(featWeightsADL$V1),as.numeric(featWeightsAIL$V1),as.numeric(featWeightsA$V1),as.numeric(featWeightsAL$V1))
# fix last four colnames
colnames(BwAgeCorTMDifDf_prWeights)[13:18]<-c('AgeDependentWeights','AgeIndependentWeights','AgeDependentWeights_Las','AgeIndependentWeights_Las','AgeWeights','AgeWeights_Las')

#y7ver
y7RidgeWeights<-ggplot(BwAgeCorTMDifDf_prWeights,aes(x=AgeDependentWeights,y=AgeIndependentWeights,color=Net1Vec)) + geom_text(size=10,label="\u25D6",family="Arial Unicode MS") + xlab('Age-Dep. EF Weight') + ylab('Age-Indep. EF Weight') + ggtitle('EF-prediction weights:Ridge')+scale_color_manual(values=ycolors,limits=Y7vec)+geom_text(aes(x=AgeDependentWeights,y=AgeIndependentWeights,color=Net2Vec),size=10,label="\u25D7",family="Arial Unicode MS")+theme_classic()
#y17ver
y17RidgeWeights<-ggplot(BwAgeCorTMDifDf_prWeights,aes(x=AgeDependentWeights,y=AgeIndependentWeights,color=Net1Vec17)) + geom_text(size=10,label="\u25D6",family="Arial Unicode MS") + xlab('Age-Dep. EF Weight') + ylab('Age-Indep. EF Weight') + ggtitle('EF-prediction weights:Ridge')+scale_color_manual(values=y17colors,limits=y17vec)+geom_text(aes(x=AgeDependentWeights,y=AgeIndependentWeights,color=Net2Vec17),size=10,label="\u25D7",family="Arial Unicode MS")+theme_minimal()
#y7ver
y7LassoWeights<-ggplot(BwAgeCorTMDifDf_prWeights,aes(x=AgeDependentWeights_Las,y=AgeIndependentWeights_Las,color=Net1Vec)) + geom_text(size=10,label="\u25D6",family="Arial Unicode MS") + xlab('Age-Dep. EF Weight') + ylab('Age-Indep. EF Weight') + ggtitle('EF-prediction weights:Lasso')+scale_color_manual(values=ycolors,limits=Y7vec)+geom_text(aes(x=AgeDependentWeights_Las,y=AgeIndependentWeights_Las,color=Net2Vec),size=10,label="\u25D7",family="Arial Unicode MS")+theme_classic()
#y17ver
y17LassoWeights<-ggplot(BwAgeCorTMDifDf_prWeights,aes(x=AgeDependentWeights_Las,y=AgeIndependentWeights_Las,color=Net1Vec17)) + geom_text(size=10,label="\u25D6",family="Arial Unicode MS") + xlab('Age-Dep. EF Weight') + ylab('Age-Indep. EF Weight') + ggtitle('EF-prediction weights:Lasso')+scale_color_manual(values=y17colors,limits=y17vec)+geom_text(aes(x=AgeDependentWeights_Las,y=AgeIndependentWeights_Las,color=Net2Vec17),size=10,label="\u25D7",family="Arial Unicode MS")+theme_minimal()

# write feature weightS with colname labels as matrices for brain mapping viz
featWeightsAD_lab<-cbind(colnames(masterdf)[indiv_bwcols_ind],featWeightsAD$V1)
featWeightsAI_lab<-cbind(colnames(masterdf)[indiv_bwcols_ind],featWeightsAI$V1)

ggarrange(y7RidgeWeights,y7LassoWeights)
ggarrange(y17RidgeWeights,y17LassoWeights)

# align with age features
#age Ridge w/ age dep ridge
AgeAgeDepRR<-ggplot(BwAgeCorTMDifDf_prWeights,aes(x=AgeWeights,y=AgeDependentWeights,color=Net1Vec)) + geom_text(size=10,label="\u25D6",family="Arial Unicode MS") + ylab('Age-Dep. EF Weight') + xlab('Age Weight') + ggtitle('Age-Dependent EF x Age Features:Ridge')+scale_color_manual(values=ycolors,limits=Y7vec)+geom_text(aes(x=AgeWeights,y=AgeDependentWeights,color=Net2Vec),size=10,label="\u25D7",family="Arial Unicode MS")+theme_minimal()+geom_smooth(method='lm', formula= y~x,color='gray82',size=2)
AgeAgeIndepRR<-ggplot(BwAgeCorTMDifDf_prWeights,aes(x=AgeWeights,y=AgeIndependentWeights,color=Net1Vec)) + geom_text(size=10,label="\u25D6",family="Arial Unicode MS") + xlab('Age Weight') + ylab('Age-Indep. EF Weight') + ggtitle('Age-Independent EF x Age Features: Ridge')+scale_color_manual(values=ycolors,limits=Y7vec)+geom_text(aes(x=AgeWeights,y=AgeIndependentWeights,color=Net2Vec),size=10,label="\u25D7",family="Arial Unicode MS")+theme_minimal()+geom_smooth(method='lm', formula= y~x,color='gray82',size=2)
ggarrange(AgeAgeDepRR,AgeAgeIndepRR)
#lasso
AgeAgeDepLas<-ggplot(BwAgeCorTMDifDf_prWeights,aes(x=AgeWeights_Las,y=AgeDependentWeights_Las,color=Net1Vec)) + geom_text(size=10,label="\u25D6",family="Arial Unicode MS") + ylab('Age-Dep. EF Weight') + xlab('Age Weight') + ggtitle('Age-Dependent EF x Age Features:Lasso')+scale_color_manual(values=ycolors,limits=Y7vec)+geom_text(aes(x=AgeWeights_Las,y=AgeDependentWeights_Las,color=Net2Vec),size=10,label="\u25D7",family="Arial Unicode MS")+theme_minimal()+geom_smooth(method='lm', formula= y~x,color='gray82',size=2)
AgeAgeIndepLas<-ggplot(BwAgeCorTMDifDf_prWeights,aes(x=AgeWeights_Las,y=AgeIndependentWeights_Las,color=Net1Vec)) + geom_text(size=10,label="\u25D6",family="Arial Unicode MS") + xlab('Age Weight') + ylab('Age-Indep. EF Weight') + ggtitle('Age-Independent EF x Age Features: Lasso')+scale_color_manual(values=ycolors,limits=Y7vec)+geom_text(aes(x=AgeWeights_Las,y=AgeIndependentWeights_Las,color=Net2Vec),size=10,label="\u25D7",family="Arial Unicode MS")+theme_minimal()+geom_smooth(method='lm', formula= y~x,color='gray82',size=2)
ggarrange(AgeAgeDepLas,AgeAgeIndepLas)
```
