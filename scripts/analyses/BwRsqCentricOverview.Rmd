...
# figure 5 flag
# motor by scale
ggplot(bwdf,aes(scalesvec,bwAgeCorVecs.avg_bw,color=domnetvec))+ scale_color_manual(values=c('#c1253c','#3281ab')) + xlab("# of Networks") + ylab(expression(paste('Age Effect (',Delta,R^2[adj],')'))) +theme_classic(base_size = 28) +guides(alpha=FALSE,color=guide_legend(title="Maximal Y7 Overlap"))+theme(legend.position='top')+geom_smooth(data=subset(bwdf,domnetvec=='Motor'),method='gam',formula = y~s(x,k=4))+geom_smooth(data=subset(bwdf,domnetvec=='DM'),method='gam',formula = y~s(x,k=4))
...
ggplot(data=mcrecnerrdf,aes(y=mreconerr,x=community_vec))+geom_line(size=3,color='#262228')+geom_point(aes(y=mreconerr,x=community_vec,alpha=select),size=9) +scale_alpha(range=c(0,1))+ scale_x_continuous(name ="Number of Networks",  breaks=seq(2,30,2))+labs(y='Reconstruction Error (a.u.)')+theme_classic(base_size = 40)+ theme(axis.title.y.left =  element_text(color = '#262228'),legend.position = 'none')
```
...
# motor by scale
ggplot(motorbwdf,aes(scalesvec,BW_R2VecEst,color=domnetvec)) + geom_point(size=6)+ scale_color_manual(values=c('#3281ab','#b61ad0','#670068')) + xlab("# of Networks") + ylab(expression(paste('EF Effect (',Delta,R^2[adj],')'))) +theme_classic(base_size = 28) +guides(alpha=FALSE,color=guide_legend(title="Maximal Y7 Overlap"))+theme(legend.position='none')+geom_smooth()
...
# figure 3 flag ### WHY ARE THE BARS GRAY NOW
