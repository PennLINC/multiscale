...
LiRat_testStat<-rep(0,b)
LiRat_testPvec<-rep(0,b)
...
for (x in 1:b){
  ...
  # fit full model
  AgeEff_by_transmodality_model<-gam(avg_bw_deltaR2~s(tmvec,k=3),data=NL_bwdf,select=F,method='REML')
  # fit reduced, intercept only
  AgeEff_intOnly_model<-gam(avg_bw_deltaR2~1,data=NL_bwdf,select=F,method='REML')
  # likelihood ratio estimate for this bootstrap (analysis of deviance)
  #LRoutput<-anova(AgeEff_intOnly_model,AgeEff_by_transmodality_model,test='LRT')
  # version using lmtest package
  logLike_reduced<-logLik(AgeEff_intOnly_model)
  logLike_full<-logLik(AgeEff_by_transmodality_model)
  # calculating difference in log likelihoods, making equiv. to chisq distrib.
  teststat <- -2 * (as.numeric(logLike_reduced)-as.numeric(logLike_full))
  LiRat_testStat[x]<-teststat
  # calculating p value from chi-square distribution
  p.val <- pchisq(teststat, df = 1, lower.tail = FALSE)
  # and saving p-values for smooth-brained non-statisticians like myself to interpret
  LiRat_testPvec[x]<-p.val
}
#### end of outer loop
plot(LiRat_testStat)
plot(LiRat_testPvec)
...
for (x in 1:b){
  ...
  tmFit<-summary(AgeEff_by_transmodality_model)$coefficients['tmvec',]
  lm_testStat[x]<-tmFit['Estimate']
  lm_testPvec[x]<-tmFit['Pr(>|t|)']
}
#### end of outer loop
plot(lm_testStat)
plot(lm_testPvec)
...
# now bootstrap x times
#for (x in 1:b){
#  sampIndices<-sample(1:464,replace=T)
#  resampDF<-bwdf[sampIndices,]
  # fit model
#  Age_net_gamRs<-gam(avg_bw_deltaR2~s(tmvec,k=3),data=resampDF)
#  sumRS<-summary(Age_net_gamRs)
#  r2adjvec[x]<-sumRS$r.sq
#  pvec[x]<-sumRS$s.pv
#}
