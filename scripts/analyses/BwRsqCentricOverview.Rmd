...
#### CHANGING THESE TO BETA COEF. INSTEAD OF DELTA R^2 - 11/20/20
...
for (i in 1:29){
  ...
  print(i)
  loopdf<-data.frame(globAgeEf$F1_Exec_Comp_Cog_Accuracy,globAgeEf$Age,globAgeEf$Sex,globAgeEf$Motion,globAgeEf[i+4])
...
ggplot(dataforplot,aes(x=seq.2..30.,y=ageTogbw,col=SigMed))+geom_step(size=2)+geom_step(aes(x=seq.2..30.,y=gbwToEF,col=SigMed))+theme_classic()+ylab('Path Coefficient')+xlab('Scale')+ggtitle('Global B/w Con. as an Age->EF Mediator')
...
#grad3
attnm<-read.csv('/cbica/projects/pinesParcels/results/aggregated_data/fc/network_grad3_overscales.csv',stringsAsFactors = F)
...
# motor by scale
ggplot(motorbwdf,aes(scalesvec,bwAgeCorVecs.avg_bw,color=domnetvec)) + geom_point(size=6)+ scale_color_manual(values=c('#3281ab','#b61ad0','#670068')) + xlab("# of Communities") + ylab(expression(paste('Age Effect (',Delta,R^2[adj],')'))) +theme_classic(base_size = 28) +guides(alpha=FALSE,color=guide_legend(title="Maximal Y7 Overlap"))+theme(legend.position='top')+geom_smooth()
# dm by scale
ggplot(DMbwdf,aes(scalesvec,bwAgeCorVecs.avg_bw,color=domnetvec)) + geom_point(size=6)+ scale_color_manual(values=c('#c1253c','#b61ad0','#670068')) + xlab("# of Communities") + ylab(expression(paste('Age Effect (',Delta,R^2[adj],')'))) +theme_classic(base_size = 28) +guides(alpha=FALSE,color=guide_legend(title="Maximal Y7 Overlap"))+theme(legend.position='top')+geom_smooth()

ggplot(DMbwdf,aes(scalesvec,bwAgeCorVecs.avg_bw,color=domnetvec17)) + geom_point(size=6)+ xlab("# of Communities") + ylab(expression(paste('Age Effect (',Delta,R^2[adj],')'))) +theme_classic(base_size = 28) +guides(alpha=FALSE,color=guide_legend(title="Maximal Y7 Overlap"))+theme(legend.position='top')+geom_smooth()
####
# b/w to unimodal
...
#           y = freq,
#           fill = response, label = response,alpha=response)) +
...
# load it in
affils=data.frame(vroom('/cbica/projects/pinesParcels/data/SingleParcellation/SingleAtlas_Analysis/group_all_Ks.csv'))
...
for (i in 2:30){
  ...
  # get number of communities here
...
#### THRESHOLD SUPER LOW VERTEX CARRYOVERS
...
ggplot(long_affilsUnique_selectScales,
  ...
  scale_fill_manual(name="Yeo 17 Overlap",values=c('#dc8303','#8d2049','#596a85','#2d9a3d','#007938','#d9e200','#bc0943','#2b1f67','#48593a','#91a967','#4183a8','#00bb89','#3245a3','#9e3ca2','#eb75b3','#68126f','#d1001c'),labels=c('Control A','Control B','Control C','Dorsal Attention A','Dorsal Attention B','Default Mode A', 'Default Mode B','Default Mode C','Limbic A','Limbic B','Somatomotor A','Somatomotor B','Temporal Parietal','Ventral Attention A','Ventral Attention B','Visual A','Visual B'))+ theme(legend.position = c(.44,-.34),legend.direction = "horizontal",plot.margin=unit(c(1,1,5,1),"cm"),legend.text=element_text(size=25))+scale_x_continuous(breaks=c(seq(2,30,by=2)))
# formnat from recon error plot for matching
# FOR FIGURE 1
selectvec<-rep(0,29)
selectvec[3]<-1
selectvec[6]<-1
...
ggplot(data=mcrecnerrdf,aes(y=mreconerr,x=community_vec))+geom_line(size=3,color='#262228')+geom_point(aes(y=mreconerr,x=community_vec,alpha=select),size=9) +scale_alpha(range=c(0,1))+ scale_x_continuous(name ="Number of Communitites",  breaks=seq(2,30,2))+labs(y='Reconstruction Error (a.u.)')+theme_classic(base_size = 40)+ theme(axis.title.y.left =  element_text(color = '#262228'),legend.position = 'none')
```

```{r}

## animations to show community emergence over scales
#library(gganimate)
#library(gifski)
#emergence_over_scales<-ggplot(bwdf,aes(x=tmvec,y=occvec,color=domnetvec)) + geom_point(size=10) + #scale_color_manual(values=c('#007500','#c1253c','#d77d00','#b8cf86','#3281ab','#b61ad0','#670068'))+#theme_classic()
#
#emergence_over_scales_anim<-emergence_over_scales + transition_states(as.integer(scalesvec), #state_length = 10)
#
#anim_save('~/multiscale/emergence_over_scales_anim.gif',animation=emergence_over_scales_anim,render=gifski_renderer(),nframes=100,fps=3)
```
...
InterceptVector<-rep(0,length=length(masterdf[,indiv_nsegcols_ind]))
...
for (i in 1:(length(bwAvgCondf)-4)){
  ...
  InterceptVector[i]<-summary(igam)$p.coeff['(Intercept)']
...
ggplot(motorbwdfMed,aes(Scale,AB_Est,color=domnetvec)) + geom_point(size=6)+ scale_color_manual(values=c('#3281ab','#b61ad0','#670068')) + xlab("# of Communities") + ylab('AB Path Coefficient') +theme_classic(base_size = 28) +guides(alpha=FALSE,color=guide_legend(title="Maximal Y7 Overlap"))+theme(legend.position='none')+geom_smooth()
...
Bwspan<-ggplot(LongAgeSpan_plotdf2[derivSigWholeSpline,],aes(agespans,tmvec,color=derivs,group=CIgroupingInd)) +geom_line(size=2) +labs(x = 'Age', y = 'Transmodality') +theme_classic(base_size = 28)+ xlim(c(8,23))+ scale_colour_gradientn(colours=c('#0b0bff','#8585ff','white','#ff9191','#ff0a0a'),values = c(0,.4,.5,.6,1),breaks=breaks, labels = breaks,limits=c(-.006,0.006),name="Change Per Year")+theme(legend.position="top",legend.key.width = unit(3, "cm"))+ylim(c(min(tmvec),max(tmvec)))
...
bwR2EFdf<-data.frame(tmvec,scalesvec,domnetvec,netpropvec,BW_R2VecEst,BW_R2VecEst_ageless,occvec,attnmvec,domnetSig,domnetAgelessSig)

raw<-ggplot(bwR2EFdf,aes(tmvec,BW_R2VecEst,color=domnetvec,alpha=netpropvec^2)) + scale_color_manual(values=c('#007500','#c1253c','#d77d00','#b8cf86','#3281ab','#b61ad0','#670068','gray')) + xlab("Transmodality") + ylab(expression(paste(Delta,R^2[adj]))) +theme_classic(base_size = 40) + geom_point(size=7,aes(tmvec,BW_R2VecEst)) + ggtitle('EF Effect by PG1')+guides(alpha=FALSE,color=guide_legend(title="Maximal Y7 Overlap"))+theme(legend.position="top")
...
# motor by scale
ggplot(motorbwdf,aes(scalesvec,BW_R2VecEst,color=domnetvec)) + geom_point(size=6)+ scale_color_manual(values=c('#3281ab','#b61ad0','#670068')) + xlab("# of Communities") + ylab(expression(paste('EF Effect (',Delta,R^2[adj],')'))) +theme_classic(base_size = 28) +guides(alpha=FALSE,color=guide_legend(title="Maximal Y7 Overlap"))+theme(legend.position='none')+geom_smooth()
...
# use bwAvgCon to get each communities' avg. connectivity
...
occdifvec=rep(0,length(colnames(individ_scalebybw_df)))
all3pgDifvec=rep(0,length(colnames(individ_scalebybw_df)))
...
EF_Bvec=rep(0,length(colnames(individ_scalebybw_df)))
EF_BPvec=rep(0,length(colnames(individ_scalebybw_df)))
...
for (i in 1:length(colnames(individ_scalebybw_df))){
    ...
    net1tmvec[i]<-tm1
    net2tmvec[i]<-tm2
    oc1=occvec[Kind[as.numeric(net1)]]
    oc2=occvec[Kind[as.numeric(net2)]]
    atn1=attnmvec[Kind[as.numeric(net1)]]
    atn2=attnmvec[Kind[as.numeric(net2)]]
    ...
    occdif=abs(oc1-oc2)
    # more like pyTHUGoras 
    pg3dif=sqrt(((tm1-tm2)^2)+((oc1-oc2)^2)+((atn1-atn2)^2))
...
## merged vectors
BwAgeCorTMDifDf<-data.frame(tmdifvec,ageEfvec,noAge_EFEfVec,EFEfvec,scalevec,Net1Vec,Net2Vec,Net1Vec17,Net2Vec17,fdr_ageEfPvec,fdr_EFEfPvec,fdr_noAge_EFEfPVec,ageDR2vec,occdifvec,all3pgDifvec,domnetSig1,domnetSig2,domnetAgelessSig1,domnetAgelessSig2,domnetSigAge1,domnetSigAge2,net1tmvec,net2tmvec)
# fdr age dr2
BwAgeCorTMDifDf$ageDR2Pvec<-p.adjust(ageDR2Pvec,method='fdr')
...
doubleBwAgeCorTMDifDf<-rbind(BwAgeCorTMDifDf,BwAgeCorTMDifDf2)
g2 <- gam(ageDR2vec ~ s(net1tmvec,net2tmvec),data = doubleBwAgeCorTMDifDf)
b<-getViz(g)
plot(sm(b, 1)) + l_fitRaster() + l_fitContour() +scale_fill_gradient2(low = "blue",high = "red",mid = "white",midpoint = 0)+labs(fill = "Age Effect (dR2)")+ggtitle("")+xlab("Transmodality (node A)")+ylab("Transmodality (node B)")
...
ggplot(data = BwAgeCorTMDifDf, aes(x = factor(scalevec), y = tmdifvec)) +
  geom_violin(alpha = .8) +
  geom_point(size = 1, alpha = 0.8)+theme_classic()+ylab('Gradient Distance')+xlab("# of Communities")

# get distribution of tm differences for bwspan plot - can focus on just one hump maybe
hist(tmdifvec,breaks=200)

# jitter added manually - one more loop over to space communities that got respaced to be proxmiate to each other
for (i in 1:length(colnames(individ_scalebybw_df))){
  curcolname<-colnames(individ_scalebybw_df)[i]
  splitname<-unlist(strsplit(curcolname,'_'))
  scalefield=splitname[4]
...
# figure 3 flag
Bwspan<-ggplot(LongAgeSpan_plotdf2[derivSigWholeSpline,],aes(agespans,tmvecJit,color=derivs,group=CIgroupingInd,alpha=AlphaSig)) +geom_line(size=3) +labs(x = 'Age', y = 'Transmodality') +theme_classic(base_size = 40)+ xlim(c(8,23))+ scale_colour_gradientn(colours=c('#0b0bff','#8585ff','white','#ff9191','#ff0a0a'),values = c(0,.4,.5,.6,1),breaks=breaks, labels = breaks,limits=c(-.06,0.06),name="Change Per Year (B/w Con.)")+theme(legend.text=element_text(size=30),legend.title = element_text(size=30),legend.position="bottom",legend.key.width = unit(3, "cm"))+ylim(c(min(tmvecJit),max(tmvecJit)))+guides(alpha=FALSE,color = guide_colorbar(title.position="top",ticks.colour = "gray",
                                ticks.linewidth = 8))+geom_vline(xintercept = c(10,15,20),linetype='dashed')+ theme(legend.margin=margin(b=-.5,t = -.5, unit='cm')) + scale_x_continuous(
    limits = c(8, 23),
    expand = c(0, 0),
    breaks = seq(8,23,by=3)
  ) +
  theme(plot.margin=unit(c(.2,.6,.5,.2),"cm"))+scale_y_continuous(expand=c(0,.1))
...
TMdifEF<-ggplot(BwAgeCorTMDifDf,aes(x=tmdifvec,y=noAge_EFEfVec,color=Net1Vec)) + geom_text(size=5,label="\u25D6",family="Arial Unicode MS") + xlab('Transmodality Difference') + ylab('EF Partial Correlation') + ggtitle('Pairwise Between-Network EF-FC Relations')+scale_color_manual(values=ycolors,limits=Y7vec)+geom_text(aes(x=tmdifvec,y=noAge_EFEfVec,color=Net2Vec),size=5,label="\u25D7",family="Arial Unicode MS")+theme_classic()+ stat_smooth(aes(group = 1),method = "gam", formula = y ~ s(x), size = 1)

OccdifEF<-ggplot(BwAgeCorTMDifDf,aes(x=occdifvec,y=noAge_EFEfVec,color=Net1Vec)) + geom_text(size=5,label="\u25D6",family="Arial Unicode MS") + xlab('Occipality Difference') + ylab('EF Partial Correlation') + ggtitle('Pairwise Between-Network EF-FC Relations')+scale_color_manual(values=ycolors,limits=Y7vec)+geom_text(aes(x=occdifvec,y=noAge_EFEfVec,color=Net2Vec),size=5,label="\u25D7",family="Arial Unicode MS")+theme_classic()+ stat_smooth(aes(group = 1),method = "gam", formula = y ~ s(x), size = 1)

GD_EF<-ggplot(BwAgeCorTMDifDf,aes(x=all3pgDifvec,y=noAge_EFEfVec,color=Net1Vec)) + geom_text(size=5,label="\u25D6",family="Arial Unicode MS") + xlab('Gradient Distance') + ylab('EF Partial Correlation') + ggtitle('Pairwise Between-Network EF-FC Relations')+scale_color_manual(values=ycolors,limits=Y7vec)+geom_text(aes(x=all3pgDifvec,y=noAge_EFEfVec,color=Net2Vec),size=5,label="\u25D7",family="Arial Unicode MS")+theme_classic()+ stat_smooth(aes(group = 1),method = "gam", formula = y ~ s(x), size = 1)

#### Age controlled versions of same plots
TMdifEF_agc<-ggplot(BwAgeCorTMDifDf,aes(x=tmdifvec,y=EFEfvec,color=Net1Vec)) + geom_text(size=5,label="\u25D6",family="Arial Unicode MS") + xlab('Transmodality Difference') + ylab('EF Partial Correlation') + ggtitle('Pairwise Between-Network EF-FC Relations:AgC')+scale_color_manual(values=ycolors,limits=Y7vec)+geom_text(aes(x=tmdifvec,y=EFEfvec,color=Net2Vec),size=5,label="\u25D7",family="Arial Unicode MS")+theme_classic()+ stat_smooth(aes(group = 1),method = "gam", formula = y ~ s(x), size = 1)

OccdifEF_agc<-ggplot(BwAgeCorTMDifDf,aes(x=occdifvec,y=EFEfvec,color=Net1Vec)) + geom_text(size=5,label="\u25D6",family="Arial Unicode MS") + xlab('Occipality Difference') + ylab('EF Partial Correlation') + ggtitle('Pairwise Between-Network EF-FC Relations:AgC')+scale_color_manual(values=ycolors,limits=Y7vec)+geom_text(aes(x=occdifvec,y=EFEfvec,color=Net2Vec),size=5,label="\u25D7",family="Arial Unicode MS")+theme_classic()+ stat_smooth(aes(group = 1),method = "gam", formula = y ~ s(x), size = 1)

GD_EF_agc<-ggplot(BwAgeCorTMDifDf,aes(x=all3pgDifvec,y=EFEfvec,color=Net1Vec)) + geom_text(size=5,label="\u25D6",family="Arial Unicode MS") + xlab('Gradient Distance') + ylab('EF Partial Correlation') + ggtitle('Pairwise Between-Network EF-FC Relations:AgC')+scale_color_manual(values=ycolors,limits=Y7vec)+geom_text(aes(x=all3pgDifvec,y=EFEfvec,color=Net2Vec),size=5,label="\u25D7",family="Arial Unicode MS")+theme_classic()+ stat_smooth(aes(group = 1),method = "gam", formula = y ~ s(x), size = 1)

OccdifEF17<-ggplot(BwAgeCorTMDifDf,aes(x=occdifvec,y=EFEfvec,color=Net1Vec17)) + geom_text(size=5,label="\u25D6",family="Arial Unicode MS") + xlab('Occipality Difference') + ylab('EF Partial Correlation') + ggtitle('Pairwise Between-Network EF-FC Relations')+scale_color_manual(values=y17colors,limits=y17vec)+geom_text(aes(x=occdifvec,y=EFEfvec,color=Net2Vec17),size=5,label="\u25D7",family="Arial Unicode MS")+theme_classic()+ stat_smooth(aes(group = 1),method = "gam", formula = y ~ s(x), size = 1)
...
agF_y7<-ggplot(ageAligneddf,aes(x=scalevec,y=noAge_EFEfVec,color=Net1Vec)) + geom_text(size=10,label="\u25D6",family="Arial Unicode MS",position=position_jitter(width=.2,height=.2)) + xlab('Scale') + ylab('EF Partial Correlation') + ggtitle('Pairwise Between-Network Age-aligned EF-FC Relations')+scale_color_manual(values=ycolors,limits=Y7vec)+geom_text(aes(x=scalevec,y=noAge_EFEfVec,color=Net2Vec),size=10,label="\u25D7",family="Arial Unicode MS",position=position_jitter(width=.2,height=.2))+theme_classic()+ylim(-.3,.3)

agM_y7<-ggplot(ageMitigateddf,aes(x=scalevec,y=EFEfvec,color=Net1Vec)) + geom_text(size=10,label="\u25D6",family="Arial Unicode MS") + xlab('Transmodality Difference') + ylab('EF Partial Correlation') + ggtitle('Pairwise Between-Network Age-mitigated EF-FC Relations')+scale_color_manual(values=ycolors,limits=Y7vec)+geom_text(aes(x=scalevec,y=EFEfvec,color=Net2Vec),size=10,label="\u25D7",family="Arial Unicode MS")+theme_classic()+ylim(-.4,.4)

agM_y7_age_relations<-ggplot(ageMitigateddf,aes(x=scalevec,y=ageEfvec,color=Net1Vec)) + geom_text(size=10,label="\u25D6",family="Arial Unicode MS") + xlab('Transmodality Difference') + ylab('Age Partial Correlation') + ggtitle('Pairwise Between-Network EF-Age-mitigated Age-FC Relations')+scale_color_manual(values=ycolors,limits=Y7vec)+geom_text(aes(x=scalevec,y=ageEfvec,color=Net2Vec),size=10,label="\u25D7",family="Arial Unicode MS")+theme_classic()+xlim(0,12)+ylim(-.4,.4)+geom_smooth(method='lm', formula= y~x,color='gray82',size=2)

# more thresholding
agF_y7_thresh<-ggplot(ageAligneddf_stringent,aes(x=scalevec,y=noAge_EFEfVec,color=Net1Vec)) + geom_text(size=10,label="\u25D6",family="Arial Unicode MS") + xlab('Transmodality Difference') + ylab('EF Partial Correlation') + ggtitle('Pairwise Between-Network Age-aligned EF-FC Relations')+scale_color_manual(values=ycolors,limits=Y7vec)+geom_text(aes(x=scalevec,y=noAge_EFEfVec,color=Net2Vec),size=10,label="\u25D7",family="Arial Unicode MS")+theme_classic()+xlim(0,12)+ylim(-.3,.3)

agM_y7_thresh<-ggplot(ageMitigateddf_stringent,aes(x=scalevec,y=EFEfvec,color=Net1Vec)) + geom_text(size=10,label="\u25D6",family="Arial Unicode MS") + xlab('Transmodality Difference') + ylab('EF Partial Correlation') + ggtitle('Pairwise Between-Network Age-mitigated EF-FC Relations')+scale_color_manual(values=ycolors,limits=Y7vec)+geom_text(aes(x=scalevec,y=EFEfvec,color=Net2Vec),size=10,label="\u25D7",family="Arial Unicode MS")+theme_classic()+xlim(0,12)+ylim(-.3,.3)
...
ggplot(BwAgeCorTMDifDf[BwAgeCorTMDifDf$ageDR2Pvec<0.05,],aes(x=tmdifvec,y=ageDR2vec))+ geom_text(size=7,label="\u25D6",family="Arial Unicode MS",aes(color=Net1Vec))+scale_color_manual(values=ycolors,limits=Y7vec)+geom_text(aes(x=tmdifvec,y=ageDR2vec,color=Net2Vec),size=7,label="\u25D7",family="Arial Unicode MS")+theme(legend.position = "none") + xlab('Transmodality Difference') + ylab(expression(paste('Age Effect(',Delta,R^2[adj],')')))+theme_classic(base_size = 35)+geom_point(data=BwAgeCorTMDifDf[BwAgeCorTMDifDf$ageDR2Pvec>0.05,],aes(x=tmdifvec,y=ageDR2vec,fill=NULL),pch=21,color='grey',size=3)+theme(legend.position = "none")+geom_smooth(size=3,method='lm',color='black',formula=y~x)
...
ggplot(data = BwAgeCorTMDifDf, aes(x = factor(scalevec), y = tmdifvec)) +
  geom_violin(alpha = .1)+ylab('Gradient Distance')+xlab("# of Communities") +theme_classic()+geom_point(data=suppl_df,aes(x=factor(scalevec),y=avgtmdif,color=gbwAge),size=3)+ scale_colour_gradientn(colours=c('#0b0bff','#8585ff','gray90','#ff9191','#ff0a0a'),values = c(0,.45,.67,.7,1),breaks=c(-.2,0,.1), labels = c(-.2,0,.1),limits=c(-.2,0.1),name=expression(paste(Delta,R^2[adj])))+theme(legend.position="top",legend.key.width = unit(3, "cm"))
...
#ggplot(data = BwAgeCorTMDifDf, aes(x = factor(scalevec), y = tmdifvec,color=ageEfvec)) +geom_point(size=5,alpha=.4)+ylab('Gradient Distance')+xlab("# of Communities") +theme_classic(base_size=28)+ scale_colour_gradientn(colours=c('#0b0bff','#8585ff','white','#ff9191','#ff0a0a'),values = c(0,.21,.46,.73,1),breaks=c(-.3,0,.3), labels = c(-.3,0,.3),limits=c(-.32,0.37),name=expression(paste(Delta,R^2[adj])))+geom_violin(alpha=0)

### can use this to viz number of edges over scales if you want (1100 width 12k height)
#ggplot(data = BwAgeCorTMDifDf, aes(x = factor(scalevec), y = tmdifvec,color=ageEfvec)) +ylab('Gradient Distance')+xlab("# of Communities") +theme_classic(base_size=28)+ scale_colour_gradientn(colours=c('#0b0bff','#8585ff','white','#ff9191','#ff0a0a'),values = c(0,.21,.46,.73,1),breaks=c(-.3,0,.3), labels = c(-.3,0,.3),limits=c(-.32,0.37),name=expression(paste(Delta,R^2[adj])))+geom_dotplot(alpha=1)


### going with this one for figure 5 draft1, 11/18/20
ggplot(data = BwAgeCorTMDifDf[1:56,], aes(x = factor(scalevec), y = tmdifvec,color=ageEfvec)) +geom_point(size=8,alpha=.5)+ylab('Gradient Distance')+xlab("# of Communities") +theme_classic(base_size=28)+ scale_colour_gradientn(colours=c('#0b0bff','#8585ff','gray90','#ff9191','#ff0a0a'),values = c(0,0.3,0.5,0.7,1),breaks=c(-.3,0,.3), labels = c(-.3,0,.3),limits=c(-.35,0.35),name=expression(paste(Delta,R^2[adj])))
...
```{r}
# section to make a table of 
# WITH PG2 - 2 - 2 -2 -2 -2 -2 -2 -2#
#| ef + age + | ef + age - | EF + no age
#| ef - age + | ef - age - | EF - no age

# plot em!
## merged vectors ### writing over other data
BwAgeDR2TMDifDf$occdifvec<-occdifvec

posEposAy7<-ggplot(BwAgeDR2TMDifDf[posEposA,],aes(x=occdifvec,y=EFDR2vec,color=Net1Vec)) + geom_text(size=9,label="\u25D6",family="Arial Unicode MS") + xlab('Occipality Difference') + ylab('EF delta r^2') + ggtitle('EF+ Age+')+scale_color_manual(values=ycolors,limits=Y7vec)+geom_text(aes(x=occdifvec,y=EFDR2vec,color=Net2Vec),size=9,label="\u25D7",family="Arial Unicode MS")+theme_classic()+xlim(0,12)

negEposAy7<-ggplot(BwAgeDR2TMDifDf[negEposA,],aes(x=occdifvec,y=EFDR2vec,color=Net1Vec)) + geom_text(size=9,label="\u25D6",family="Arial Unicode MS") + xlab('Occipality Difference') + ylab('EF delta r^2') + ggtitle('EF- Age+')+scale_color_manual(values=ycolors,limits=Y7vec)+geom_text(aes(x=occdifvec,y=EFDR2vec,color=Net2Vec),size=9,label="\u25D7",family="Arial Unicode MS")+theme_classic()+xlim(0,12)

posEnegAy7<-ggplot(BwAgeDR2TMDifDf[posEnegA,],aes(x=occdifvec,y=EFDR2vec,color=Net1Vec)) + geom_text(size=9,label="\u25D6",family="Arial Unicode MS") + xlab('Occipality Difference') + ylab('EF delta r^2') + ggtitle('EF+ Age-')+scale_color_manual(values=ycolors,limits=Y7vec)+geom_text(aes(x=occdifvec,y=EFDR2vec,color=Net2Vec),size=9,label="\u25D7",family="Arial Unicode MS")+theme_classic()+xlim(0,12)

negEnegAy7<-ggplot(BwAgeDR2TMDifDf[negEnegA,],aes(x=occdifvec,y=EFDR2vec,color=Net1Vec)) + geom_text(size=9,label="\u25D6",family="Arial Unicode MS") + xlab('Occipality Difference') + ylab('EF delta r^2') + ggtitle('EF- Age-')+scale_color_manual(values=ycolors,limits=Y7vec)+geom_text(aes(x=occdifvec,y=EFDR2vec,color=Net2Vec),size=9,label="\u25D7",family="Arial Unicode MS")+theme_classic()+xlim(0,12)

posEnoAy7<-ggplot(BwAgeDR2TMDifDf[posEnoA,],aes(x=occdifvec,y=EFDR2vec,color=Net1Vec)) + geom_text(size=9,label="\u25D6",family="Arial Unicode MS") + xlab('Occipality Difference') + ylab('EF delta r^2') + ggtitle('EF+ No Age')+scale_color_manual(values=ycolors,limits=Y7vec)+geom_text(aes(x=occdifvec,y=EFDR2vec,color=Net2Vec),size=9,label="\u25D7",family="Arial Unicode MS")+theme_classic()+xlim(0,12)

negEnoAy7<-ggplot(BwAgeDR2TMDifDf[negEnoA,],aes(x=occdifvec,y=EFDR2vec,color=Net1Vec)) + geom_text(size=9,label="\u25D6",family="Arial Unicode MS") + xlab('Occipality Difference') + ylab('EF delta r^2') + ggtitle('EF- No Age')+scale_color_manual(values=ycolors,limits=Y7vec)+geom_text(aes(x=occdifvec,y=EFDR2vec,color=Net2Vec),size=9,label="\u25D7",family="Arial Unicode MS")+theme_classic()+xlim(0,12)

############## 17 coloration
posEposAy17<-ggplot(BwAgeDR2TMDifDf[posEposA,],aes(x=occdifvec,y=EFDR2vec,color=Net1Vec17)) + geom_text(size=9,label="\u25D6",family="Arial Unicode MS") + xlab('Occipality Difference') + ylab('EF delta r^2') + ggtitle('EF+ Age+')+scale_color_manual(values=y17colors,limits=y17vec)+geom_text(aes(x=occdifvec,y=EFDR2vec,color=Net2Vec17),size=9,label="\u25D7",family="Arial Unicode MS")+theme_classic()+xlim(0,12)

negEposAy17<-ggplot(BwAgeDR2TMDifDf[negEposA,],aes(x=occdifvec,y=EFDR2vec,color=Net1Vec17)) + geom_text(size=9,label="\u25D6",family="Arial Unicode MS") + xlab('Occipality Difference') + ylab('EF delta r^2') + ggtitle('EF- Age+')+scale_color_manual(values=y17colors,limits=y17vec)+geom_text(aes(x=occdifvec,y=EFDR2vec,color=Net2Vec17),size=9,label="\u25D7",family="Arial Unicode MS")+theme_classic()+xlim(0,12)

posEnegAy17<-ggplot(BwAgeDR2TMDifDf[posEnegA,],aes(x=occdifvec,y=EFDR2vec,color=Net1Vec17)) + geom_text(size=9,label="\u25D6",family="Arial Unicode MS") + xlab('Occipality Difference') + ylab('EF delta r^2') + ggtitle('EF+ Age-')+scale_color_manual(values=y17colors,limits=y17vec)+geom_text(aes(x=occdifvec,y=EFDR2vec,color=Net2Vec17),size=9,label="\u25D7",family="Arial Unicode MS")+theme_classic()+xlim(0,12)

negEnegAy17<-ggplot(BwAgeDR2TMDifDf[negEnegA,],aes(x=occdifvec,y=EFDR2vec,color=Net1Vec17)) + geom_text(size=9,label="\u25D6",family="Arial Unicode MS") + xlab('Occipality Difference') + ylab('EF delta r^2') + ggtitle('EF- Age-')+scale_color_manual(values=y17colors,limits=y17vec)+geom_text(aes(x=occdifvec,y=EFDR2vec,color=Net2Vec17),size=9,label="\u25D7",family="Arial Unicode MS")+theme_classic()+xlim(0,12)

posEnoAy17<-ggplot(BwAgeDR2TMDifDf[posEnoA,],aes(x=occdifvec,y=EFDR2vec,color=Net1Vec17)) + geom_text(size=9,label="\u25D6",family="Arial Unicode MS") + xlab('Occipality Difference') + ylab('EF delta r^2') + ggtitle('EF+ No Age')+scale_color_manual(values=y17colors,limits=y17vec)+geom_text(aes(x=occdifvec,y=EFDR2vec,color=Net2Vec17),size=9,label="\u25D7",family="Arial Unicode MS")+theme_classic()+xlim(0,12)

negEnoAy17<-ggplot(BwAgeDR2TMDifDf[negEnoA,],aes(x=occdifvec,y=EFDR2vec,color=Net1Vec17)) + geom_text(size=9,label="\u25D6",family="Arial Unicode MS") + xlab('Occipality Difference') + ylab('EF delta r^2') + ggtitle('EF- No Age')+scale_color_manual(values=y17colors,limits=y17vec)+geom_text(aes(x=occdifvec,y=EFDR2vec,color=Net2Vec17),size=9,label="\u25D7",family="Arial Unicode MS")+theme_classic()+xlim(0,12)


############## Scale coloration

posEposAs<-ggplot(BwAgeDR2TMDifDf[posEposA,],aes(x=occdifvec,y=EFDR2vec,color=scalevec)) + geom_text(size=9,label="\u25D6",family="Arial Unicode MS") + xlab('Occipality Difference') + ylab('EF delta r^2') + ggtitle('EF+ Age+')+scale_color_gradientn(colours = rainbow(5))+geom_text(aes(x=occdifvec,y=EFDR2vec,color=scalevec),size=9,label="\u25D7",family="Arial Unicode MS")+theme_classic()+xlim(0,12)

negEposAs<-ggplot(BwAgeDR2TMDifDf[negEposA,],aes(x=occdifvec,y=EFDR2vec,color=scalevec)) + geom_text(size=9,label="\u25D6",family="Arial Unicode MS") + xlab('Occipality Difference') + ylab('EF delta r^2') + ggtitle('EF- Age+')+scale_color_gradientn(colours = rainbow(5))+geom_text(aes(x=occdifvec,y=EFDR2vec,color=scalevec),size=9,label="\u25D7",family="Arial Unicode MS")+theme_classic()+xlim(0,12)

posEnegAs<-ggplot(BwAgeDR2TMDifDf[posEnegA,],aes(x=occdifvec,y=EFDR2vec,color=scalevec)) + geom_text(size=9,label="\u25D6",family="Arial Unicode MS") + xlab('Occipality Difference') + ylab('EF delta r^2') + ggtitle('EF+ Age-')+scale_color_gradientn(colours = rainbow(5))+geom_text(aes(x=occdifvec,y=EFDR2vec,color=scalevec),size=9,label="\u25D7",family="Arial Unicode MS")+theme_classic()+xlim(0,12)

negEnegAs<-ggplot(BwAgeDR2TMDifDf[negEnegA,],aes(x=occdifvec,y=EFDR2vec,color=scalevec)) + geom_text(size=9,label="\u25D6",family="Arial Unicode MS") + xlab('Occipality Difference') + ylab('EF delta r^2') + ggtitle('EF- Age-')+scale_color_gradientn(colours = rainbow(5))+geom_text(aes(x=occdifvec,y=EFDR2vec,color=scalevec),size=9,label="\u25D7",family="Arial Unicode MS")+theme_classic()+xlim(0,12)

posEnoAs<-ggplot(BwAgeDR2TMDifDf[posEnoA,],aes(x=occdifvec,y=EFDR2vec,color=scalevec)) + geom_text(size=9,label="\u25D6",family="Arial Unicode MS") + xlab('Occipality Difference') + ylab('EF delta r^2') + ggtitle('EF+ No Age')+scale_color_gradientn(colours = rainbow(5))+geom_text(aes(x=occdifvec,y=EFDR2vec,color=scalevec),size=9,label="\u25D7",family="Arial Unicode MS")+theme_classic()+xlim(0,12)

negEnoAs<-ggplot(BwAgeDR2TMDifDf[negEnoA,],aes(x=occdifvec,y=EFDR2vec,color=scalevec)) + geom_text(size=9,label="\u25D6",family="Arial Unicode MS") + xlab('Transmodality Difference') + ylab('EF delta r^2') + ggtitle('EF- No Age')+scale_color_gradientn(colours = rainbow(5))+geom_text(aes(x=occdifvec,y=EFDR2vec,color=scalevec),size=9,label="\u25D7",family="Arial Unicode MS")+theme_classic()+xlim(0,12)

#megaPosEf<-ggarrange(posEposAy7,posEnegAy7,posEnoAy7,posEposAy17,posEnegAy17,posEnoAy17,posEposAs,posEnegAs,posEnoAs)
#megaNegEf<-ggarrange(negEposAy7,negEnegAy7,negEnoAy7,negEposAy17,negEnegAy17,negEnoAy17,negEposAs,negEnegAs,negEnoAs)


#plot(KAcrossPos$V1,BwAgeDR2TMDifDf$tmdifvec)
```
...
#| ef raw scale
######fdr'ed EF
#####fdrEFDr2Ps<-p.adjust(EFDeltaPEstVec_agelessvec,method='fdr')
###### index of features surviving FDR
#####EFsurvivors<-which(fdrEFDr2Ps<0.05)
#####EFsurivor_EF_Effects<-EFDR2vec[EFsurvivors]
#####EFsurivor_Age_Effects<-ageDR2vec[EFsurvivors]
...
# plot em!
## merged vectors
BwAgeDR2TMDifDf<-data.frame(tmdifvec,scalevec,Net1Vec,Net2Vec,Net1Vec17,Net2Vec17,ageDR2vec,EFDeltaEstVec_agelessvec)
...
all_agelessEF<-ggplot(BwAgeDR2TMDifDf,aes(x=occdifvec,y=EFDeltaEstVec_agelessvec,color=Net1Vec)) + geom_text(size=9,label="\u25D6",family="Arial Unicode MS") + xlab('Occipality Difference') + ylab('EF delta r^2') + ggtitle('EF- No Age')+scale_color_manual(values=ycolors,limits=Y7vec)+geom_text(aes(x=occdifvec,y=EFDeltaEstVec_agelessvec,color=Net2Vec),size=9,label="\u25D7",family="Arial Unicode MS")+theme_classic()+xlim(0,12)

############## 17 coloration
all_agelessEF17<-ggplot(BwAgeDR2TMDifDf,aes(x=occdifvec,y=EFDeltaEstVec_agelessvec,color=Net1Vec17)) + geom_text(size=9,label="\u25D6",family="Arial Unicode MS") + xlab('Occipality Difference') + ylab('EF delta r^2') + ggtitle('EF effect without controlling for Age')+scale_color_manual(values=y17colors,limits=y17vec)+geom_text(aes(x=occdifvec,y=EFDeltaEstVec_agelessvec,color=Net2Vec17),size=9,label="\u25D7",family="Arial Unicode MS")+theme_classic()+xlim(0,12)+ stat_smooth(method = "gam", formula = y ~ s(x), size = 1)
```
...
BW_interceptVec<-rep(0,464)
BW_deriv10vec<-rep(0,464)
BW_deriv15vec<-rep(0,464)
BW_deriv20vec<-rep(0,464)

# use calculated InterceptVector for intercept
BW_interceptVec<-InterceptVector
...
# 15 y.o. 82/178 months in
deriv.point.15<-round((82/178)*200)
# 20 y.o. is 142/178 months in
deriv.point.20<-round((142/178)*200)
#derivInfo
BW_deriv10vec<-derivInfo[,deriv.point.10]
BW_deriv15vec<-derivInfo[,deriv.point.15]
BW_deriv20vec<-derivInfo[,deriv.point.20]
...
for (K in 2:30){
      ...
      K_end=(((K-1)*(K))/2)+K-1
      Kind<-K_start:K_end
      Intercept=InterceptVector[Kind]
      at10=BW_deriv10vec[Kind]
      at15=BW_deriv15vec[Kind]
      at20=BW_deriv20vec[Kind]
      # write em out to cubic
      write.table(Intercept,paste('/cbica/projects/pinesParcels/results/EffectVecs/BW_InterceptAt',K,sep=''),sep=',', col.names = F,quote = F,row.names=F)
      ...
      write.table(at15,paste('/cbica/projects/pinesParcels/results/EffectVecs/Deriv15yoAt',K,sep=''),sep=',', col.names = F,quote = F,row.names=F)
      write.table(at20,paste('/cbica/projects/pinesParcels/results/EffectVecs/Deriv20yoAt',K,sep=''),sep=',', col.names = F,quote = F,row.names=F)
}
...
forRRfc<-cbind(masterdf$Age,masterdf$Motion,masterdf[,scaleind])
RRfcFN<-paste('/cbica/projects/pinesParcels/results/EffectVecs/scale31for_bw_RRfc.csv',sep='')
write.table(forRRfc,RRfcFN,row.names = F,col.names = F,sep = ',')
######### Adapted forEF
for (k in 2:30){
# grep out edges at this scale
...
# write em out to cubic
write.table(edgesAtK_EF,paste('/cbica/projects/pinesParcels/results/EffectVecs/edgesAt',k,'_EF',sep=''),sep=',', col.names = F,quote = F,row.names=F)
}
...
write.table(edges_EF,'/cbica/projects/pinesParcels/results/EffectVecs/edgesAt31_EF',sep=',', col.names = F,quote = F,row.names=F)
```
...
# czech out the predicted EF values
predEFcsv<-read.csv('/cbica/projects/pinesParcels/data/aggregated_data/SubjPreds_MS.csv',header=F)
...
U<-ggplot(predDF,aes(Age,predict_U2))+geom_smooth(col='#65156EFF',size=6)+geom_smooth(aes(Age,predict_U30),col='#65156EFF',size=2)+theme_classic()+ylab('Unimodal Communities')+ylim(-.08,.08)
T<-ggplot(predDF,aes(Age,predict_T2))+geom_smooth(col='#FAC127FF',size=6)+geom_smooth(aes(Age,predict_T30),col='#FAC127FF',size=2)+theme_classic()+ylab('Transmodal Communities')+ylim(-.08,.08)
...
U<-ggplot(predDF,aes(EF,predict_U2))+geom_smooth(col='#65156EFF',size=6)+geom_smooth(aes(EF,predict_U30),col='#65156EFF',size=2)+theme_classic()+ylab('Unimodal Communities')+ylim(-.08,.08)
T<-ggplot(predDF,aes(EF,predict_T2))+geom_smooth(col='#FAC127FF',size=6)+geom_smooth(aes(EF,predict_T30),col='#FAC127FF',size=2)+theme_classic()+ylab('Transmodal Communities')+ylim(-.08,.08)
...
# figure 3A flag
ggplot(LongAgeSpan_plotdf3,aes(Age,Splines,color=Transmodality,group=Grouping)) +geom_line(size=3,alpha=.4) +labs(x = 'Age', y = 'Between-Community Coupling') +theme_classic(base_size = 40)+ scale_colour_viridis(option="inferno",guide=guide_colorbar(barwidth = 22,ticks.colour = "gray",ticks.linewidth = 8))+theme(legend.text=element_text(size=30),legend.title = element_text(size=30),legend.position="bottom",legend.key.width = unit(3, "cm"))+theme(legend.position="bottom")+ theme(legend.margin=margin(b=-.5,t = -.5, unit='cm')) + scale_x_continuous(
    limits = c(8, 23),
    expand = c(0, 0),
    breaks = seq(8,23,by=3)
  ) +
  theme(plot.margin=unit(c(.2,.6,.5,.2),"cm"))
