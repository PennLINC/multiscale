...
#  alledges[,i]<-((alledges[,i]-mean(alledges[,i])))/(sd(alledges[,i]))
#}


# combine feature matrices with EF scores
edgesAt17_EF<-cbind(edges,masteref$F1_Exec_Comp_Cog_Accuracy)
edges_EF<-cbind(alledges,masteref$F1_Exec_Comp_Cog_Accuracy)


# write em out to cubic
write.table(edgesAt17_EF,'/cbica/projects/pinesParcels/results/EffectVecs/edgesAt17_EF',sep=',', col.names = F,quote = F,row.names=F)
write.table(edges_EF,'/cbica/projects/pinesParcels/results/EffectVecs/edges_EF',sep=',', col.names = F,quote = F,row.names=F)
```
