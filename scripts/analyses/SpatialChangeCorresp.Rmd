...
ggplot(m_spinDistr[m_spinDistr$RealCor==0,], aes(x = value, y = Scale, group = Scale))+xlab(expression(rho)) +
  geom_density_ridges(scale = 10, size = 0.25, rel_min_height = 0.03,alpha=.3) +geom_point(data=m_spinDistr[m_spinDistr$RealCor==1,],aes(x=value,y=Scale))+theme_minimal()
```