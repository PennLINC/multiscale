...
outputcolnames=c('Motion','Sex','EF','logScale','Age','logScale_x_Age','EF_x_Age')
...
returnLmCoefs<-function(d,i){
  ...
  d2<-d[i,]
  model=gamm(value~Motion+Sex+log(Scale)*s(Age)+EF*s(Age),random=list(bblid=~1),data=d2,knots=3)
  return(fixef(model))
}
...
for (v in 17001:17734){
  ...
  model=gamm(value~Motion+Sex+log(Scale)*s(Age)+EF*s(Age),random=list(bblid=~1),data=mdf_verts,knots=3)
  fe=fixef(model)
  re=ranef(model)
  re=re$bblid
  ...
  # index 4 is logScale
  outarray[2,4]=boot.ci(results,type="norm",index=4)$normal[2]
  outarray[3,4]=boot.ci(results,type="norm",index=4)$normal[3]
  # index 5 is Age
  outarray[2,5]=boot.ci(results,type="norm",index=5)$normal[2]
  outarray[3,5]=boot.ci(results,type="norm",index=5)$normal[3]
  # index 6 is EF
  outarray[2,3]=boot.ci(results,type="norm",index=6)$normal[3]
  ...
  # index 8 is age*EF
  outarray[3,7]=boot.ci(results,type="norm",index=8)$normal[3]
  outarray[3,7]=boot.ci(results,type="norm",index=8)$normal[3]

  # no bootstrap equiv for int age correlation at the moment

  write.table(outarray,file=paste('~/mixedEffectModels/Modeled_fSex_fMot_fAgexScale_fAgeXEF_raInt_v',v,'_bwVals_BoStr_overScales.csv',sep=''),sep=',',row.names=F,quote=F)
  # 9/10/20 section to write out subject level intercepts and slopes
