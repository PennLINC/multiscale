...
ggplot(data = mPdfmelt, aes(x = fct_inorder(variable), y = value))+geom_jitter(aes(x=fct_inorder(variable),y=value),size=1,alpha=.3)+ylab('Median w/in - b/w ARI vs. Permuted')+xlab("Scales and rest-task pairs") +theme_classic(base_size=20)+geom_point(data=mOdfmelt,aes(x=fct_inorder(variable),y=value),size=3,alpha=1)+scale_y_continuous(limits=c(-.1,.2))
