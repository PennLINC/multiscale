...
ggplot(AgeSpan_plotdf,aes(AgeBounds,corVecEst,color=domnetvec,group=CIgroupingInd)) +geom_line(size=1) +labs(title='Age Effect Span', x = 'Span of Spline Deriv 95% CI ~= 0', y = 'AgeSegregEst') +theme_classic(base_size = 28)+ xlim(c(8,23))+ scale_color_manual(name="Predominant \nOverlapping \nYeo7 Network",values=c('#007500','#c1253c','#d77d00','#b8cf86','#3281ab','#b61ad0','#670068'))

ggplot(AgeSpan_plotdf,aes(AgeBounds,corVecEst,color=scalesvec,group=CIgroupingInd)) +geom_line(size=1) +labs(title='Age Effect Span', x = 'Span of Spline Deriv 95% CI ~= 0', y = 'AgeSegregEst') +theme_classic(base_size = 28)+ xlim(c(8,23))
...
for (i in 1:10){
  ...
  avgDeriv<-mean(derivInfoYears[n,((i*20)-19):(i*20)])
...
# Make very long deriv spanning plot with dumb code
VL_derivInfo<-data.frame(rbind(AgeSpan_plotdf,AgeSpan_plotdf,AgeSpan_plotdf,AgeSpan_plotdf,AgeSpan_plotdf,AgeSpan_plotdf,AgeSpan_plotdf,AgeSpan_plotdf,AgeSpan_plotdf,AgeSpan_plotdf))
...
VL_derivInfo$AgeBounds<-rbind(TenBinsBounds,TenBinsBounds)
VL_derivInfo$deriv<-c(derivStack,derivStack)
#+ scale_colour_gradient(low="#55185D", high="#ECB602")

a<-ggplot(VL_derivInfo,aes(AgeBounds,tmvec,color=deriv,group=CIgroupingInd)) +geom_line(size=3) +labs(title='Slope of Segreg-Age Relation Over Age Range', x = 'Span of Spline Deriv 95% CI ~= 0', y = 'Transmodality') +theme_classic(base_size = 28)+ xlim(c(8,23))+ scale_colour_gradientn(colours=c('white','blue','red','white'),values = c(0,.22,.32,1))

b<-ggplot(VL_derivInfo[1:4640,],aes(AgeBounds,corVecEst,color=deriv,group=CIgroupingInd)) +geom_line(size=3) +labs(title='Slope of Segreg-Age Relation Over Age Range', x = 'Span of Spline Deriv 95% CI ~= 0', y = 'Age Segregation Effect') +theme_classic(base_size = 28)+ xlim(c(8,23))+ scale_colour_gradientn(colours=c('white','blue','red','white'),values = c(0,.22,.32,1))
