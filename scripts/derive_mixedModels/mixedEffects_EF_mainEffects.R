...
model=gamm(value~Motion+Sex+s(Scale,k=3,fx=T)+s(Age,k=3,fx=T)+s(EF,k=3,fx=T),random=list(bblid=~1),data=mdf_verts)
...
EF_obs=gammtable['s(EF)','F']
outarray['Coef','s_EF']=EF_obs
...
EF_b = vector(mode = "numeric", length = B)
...
for (b in 1:B){
  ...
  mdf_verts<-melt(data_resamp,id=c(1,2,3,4,5,6))
  mdf_verts$bblid<-as.factor(mdf_verts$bblid)
  colnames(mdf_verts)[7]<-c('Scale')
...
EF_CI_bound1 = EF_obs - 1.96*sqrt(var(EF_b)/length(EF_b))
EF_CI_bound2 = EF_obs + 1.96*sqrt(var(EF_b)/length(EF_b))
outarray['BoStrCI_Low','s_EF']=EF_CI_bound1
outarray['BoStrCI_Upp','s_EF']=EF_CI_bound2
# now do the studentized p val calculation
if (EF_CI_bound1 > 0 | EF_CI_bound2 > 0){
  pval = 2*pnorm(max(EF_CI_bound1, EF_CI_bound2), lower.tail = F) # getting the area above the upper bound of the CI
} else {
  pval = 2*pnorm(min(EF_CI_bound1, EF_CI_bound2), lower.tail = T) # getting the area below the lower bound of the CI
}
outarray['StudentizedP','s_EF']<-pval
