...
EFDeltaR2EstVec_ageless<-function(x){
  ...
  # no-EF model (segreg ~ sex + motion)
  noEFGam<-lm(varofint~Sex+Motion,data=scaledf)
  noEFSum<-summary(noEFGam)
  # EF-included model for measuring difference
  EFGam<-lm(varofint~EF+Sex+Motion,data=scaledf)
  EFSum<-summary(EFGam)
  dif<-EFSum$r.sq-noEFSum$r.sq
  # partial spearmans to extract EF relation (for direction)
  pspear=pcor(scaledf,method='spearman')$estimate
...
EFDeltaPEstVec_ageless<-function(x){
  ...
  # no-EF model (segreg ~ sex + motion)
  noEFlm<-lm(varofint~Sex+Motion,data=scaledf)
  # ef-included model for measuring difference
  EFlm<-lm(varofint~EF+Sex+Motion,data=scaledf)
  
  # test of dif with anova.gam
  anovaRes<-anova(noEFlm,EFlm,test='Chisq')
  anovaP<-anovaRes$`Pr(>Chi)`
  anovaP2<-unlist(anovaP)
  return(anovaP2[2])
...
cBW_R2VecEst_ageless<-BW_R2VecEst_ageless
cBW_R2VecEst_ageless[corrected_ageless>0.05]=0

# with FDR corrections zeroed
cbwR2EFdf<-data.frame(tmvec,scalesvec,domnetvec,netpropvec,cBW_R2VecEst,occvec,cBW_R2VecEst_ageless)
...
# without FDR corrections zeroed
bwR2EFdf<-data.frame(tmvec,occvec,scalesvec,domnetvec,netpropvec,BW_R2VecEst)
...
## merged vectors
BwAgeCorTMDifDf<-data.frame(tmdifvec,ageEfvec,noAge_EFEfVec,EFEfvec,scalevec,Net1Vec,Net2Vec,Net1Vec17,Net2Vec17,fdr_ageEfPvec,fdr_EFEfPvec,fdr_noAge_EFEfPVec,ageDR2vec,occdifvec)
# fdr age dr2
