function subj_vert_fc_pcor(fc_configfp)
	...
	for i=2:max(Krange)
		...
		K_bTSP_house{i}=zeros(i);
		K_bTSC_house{i}=zeros(i);
	end
	...
	for K=2:max(Krange)
		...
		%Khouse{K}=Kmat;
                %GKhouse{K}=g_Kmat;
                K_bTSP_house{K}=bts_conmat;
		K_bTSC_house{K}=bts_conmatc;
		toc
	end
	% save files to subjdir
	subjmats=struct('K_bTSP_house',K_bTSP_house,'K_bTSC_house',K_bTSC_house);
