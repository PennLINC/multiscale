...
# best fit so far
model=glmer(value~Age+Motion+Scale+Sex+(1 + log(Scale)|bblid),data=mdf_MotVert)
model_ageout=glmer(value~Motion+Scale+Sex+(1 + log(Scale)|bblid),data=mdf_MotVert)
...
# check out interecepts and slopes for age*scale and without age*scale and with just scale, correlations of intercepts
...
model=glmer(value~Age+Motion+Sex+(1 + log(Scale)|bblid),data=mdf_globalVerts)
v<-visreg(model,"Scale",by="bblid",re.form=~(1 + log(Scale)|bblid),plot=F)
```