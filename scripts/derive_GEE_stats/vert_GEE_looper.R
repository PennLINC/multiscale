...
L_contrast_matcher=c('Intercept','s_Age','s_Age','s_Scale','s_Scale','AgeXScale','AgeXScale','AgeXScale','AgeXScale','Motion','Sex')
...
# only hitting Scale(2) and AgexScale(4) with this script: EFxscale will be modeled independently in another script
for (i in c(2,4)){
	...
	# 2 linear effects + 2 smooths (x2) + 1 smooth interactions (x4) (+1 for intercept?) = length 11 contrast vector
	L_contrast = rep(0,11)
	...
	fit_gam=mgcv::gam(value~Motion+Sex+s(Scale,k=3,fx=T)+s(Age,k=3,fx=T)+ti(Scale,Age,k=3,fx=T),data=mdf_verts)
	...
	fit_gee = geepack::geeglm(value~ s.Age..1 + s.Age..2 + 
			      s.Scale..1 + s.Scale..2 +
                              ...
                              ti.Scale.Age..4 +  Motion + Sex2, id = gam.model.df$bblid,data = gam.model.df, corstr = "exchangeable")
...
write.table(outarray,file=paste('~/mixedEffectModels/Modeled_GEE_Sex_Mot_s3fxT_AgexScale_v',v,'_bwVals_overScales.csv',sep=''),sep=',',row.names=F,quote=F)
