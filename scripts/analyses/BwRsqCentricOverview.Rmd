...
ggplot(long_affilsUnique,
  ...
  scale_fill_manual(values=c('#007500','#c1253c','#d77d00','#b8cf86','#3281ab','#b61ad0','#670068'))
...
```{r}
## neural network format to show community emergence over scales
...
#library(devtools)
#source_url('https://gist.githubusercontent.com/fawda123/7471137/raw/466c1474d0a505ff044412703516c34f#1a4684a5/nnet_plot_update.r')
# 
##plot each model
#plot.nnet(mod1)
#mod.in<-c(13.12,1.49,0.16,-0.11,-0.19,-0.16,0.56,-0.52,0.81,0.56,-0.52,0.81,0.56,-0.52,0.81,.1,.1,.1#)
#struct<-c(2,3,4) #two inputs, two hidden, one output 
...
# gamm version
#model=gamm(value~Motion+Sex+Scale+ti(Age,k=4,fx=T)+ti(transmodality,k=4,fx=T)+ti(Scale,Age,k=4,fx=T)+ti(Scale,transmodality,k=4,fx=T)+ti(Scale,Age,transmodality,k=4,fx=T),random=list(bblid=~1),data=melt_long)
#lmer version
...
MediationDF<-data.frame(tmvec,scalesvec,occvec,domnetvec,netpropvec,AB_Est)

all<-ggplot(MediationDF,aes(tmvec,AB_Est,color=domnetvec,alpha=netpropvec^2)) + scale_color_manual(values=c('#007500','#c1253c','#d77d00','#b8cf86','#3281ab','#b61ad0','#670068')) + xlab("Transmodality") + ylab('AB Path Coefficient') +theme_classic(base_size = 28) + geom_point(size=7,aes(tmvec,AB_Est))+guides(alpha=FALSE,color=guide_legend(title="Maximal Y7 Overlap"))+theme(legend.position="bottom")
...
Bwspan<-ggplot(LongAgeSpan_plotdf2,aes(agespans,tmvec,color=derivs,group=CIgroupingInd)) +geom_line(size=4) +labs(x = 'Age', y = 'Transmodality') +theme_classic(base_size = 28)+ xlim(c(8,23))+ scale_colour_gradientn(colours=c('#0b0bff','#8585ff','white','#ff9191','#ff0a0a'),values = c(0,.25,.5,.75,1),breaks=breaks, labels = breaks,limits=c(-.006,0.006),name="Change Per Year")+theme(legend.position="top",legend.key.width = unit(3, "cm"))+ylim(c(min(tmvec),max(tmvec)))
...
avgbw_age_tm_sig<-ggplot(bwdf[CL_sigVec,],aes(tmvec,bwAgeCorVecs.avg_bw,color=domnetvec,alpha=netpropvec^2)) + geom_point(size=6)+ scale_color_manual(values=c('#007500','#c1253c','#d77d00','#b8cf86','#3281ab','#b61ad0','#670068')) + xlab("Transmodality") + ylab(expression(paste(Delta,R^2[adj]))) +theme_classic(base_size = 28) +guides(alpha=FALSE,color=guide_legend(title="Maximal Y7 Overlap"))+theme(legend.position="top")
```
...
nonlinMod=gamm(AvgBetweenCon~s(Scale,k=4,fx=T)+s(Transmodality,k=4,fx=T)+s(Age,k=4,fx=T)+ti(Scale,Age,k=4,fx=T)+ti(Age,Transmodality,k=4,fx=T)+ti(Age,Transmodality,Scale,k=4,fx=T)+Sex+Motion,random=list(bblid=~1),data=Melted_clevel_ef)
#nonlinModOF=readRDS('~/multiscale/CommLevelnonLinAgeModelScaleOF')
#plot_model(nonlinMod,type = "pred", terms = c("Age","Transmodality[-6,6]","Scale[4,20]"))  
...
PartialnonlinModEF=gamm(AvgBetweenCon~s(Scale,k=3,fx=T)+s(Transmodality,k=3,fx=T)+s(Age,k=3,fx=T)+ti(Scale,EF,k=3,fx=T)+ti(EF,Transmodality,k=3,fx=T)+ti(EF,Scale,Transmodality,k=3,fx=T)+Sex+Motion+s(EF,k=3,fx=T),random=list(bblid=~1),data=Melted_clevel_ef)
...
# "Ageless" Ef model
DualLevelnonlinModEF=gamm(AvgBetweenCon~s(Scale,k=3,fx=T)+s(Transmodality,k=3,fx=T)+ti(Scale,EFResid,k=3,fx=T)+ti(EFResid,Transmodality,k=3,fx=T)+ti(EFResid,Scale,Transmodality,k=3,fx=T)+Sex+Motion+s(EFResid,k=3,fx=T),random=list(bblid=~1),data=Melted_clevel_ef)
