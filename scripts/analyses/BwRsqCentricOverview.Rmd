...
EFPG1_ageless<-ggplot(bwR2EFdf,aes(tmvec,BW_R2VecEst_ageless,color=domnetAgelessSig,alpha=netpropvec^2)) + scale_color_manual(values=c('#007500','#c1253c','#d77d00','#b8cf86','#3281ab','#b61ad0','#670068','gray80')) + xlab("PG1") + ylab('EF Partial Correlation') +theme_classic(base_size = 40) + geom_point(size=7,aes(tmvec,BW_R2VecEst_ageless)) + ggtitle('EF Effect by PG1: Main Effect')+guides(alpha=FALSE,color=guide_legend(title="Maximal Y7 Overlap"))+theme(legend.position="top")

EFPG2_ageless<-ggplot(bwR2EFdf,aes(occvec,BW_R2VecEst_ageless,color=domnetAgelessSig,alpha=netpropvec^2)) + scale_color_manual(values=c('#007500','#c1253c','#d77d00','#b8cf86','#3281ab','#b61ad0','#670068','gray80')) + xlab("PG2") + ylab(expression('EF Partial Correlation') +theme_classic(base_size = 40) + geom_point(size=7,aes(occvec,BW_R2VecEst_ageless)) + ggtitle('EF Effect by PG2: Main Effect')+guides(alpha=FALSE,color=guide_legend(title="Maximal Y7 Overlap"))+theme(legend.position="top")

EFPG3_ageless<-ggplot(bwR2EFdf,aes(attnmvec,BW_R2VecEst,color=domnetAgelessSig,alpha=netpropvec^2)) + scale_color_manual(values=c('#007500','#c1253c','#d77d00','#b8cf86','#3281ab','#b61ad0','#670068','gray80')) + xlab("PG3") + ylab('EF Partial Correlation') +theme_classic(base_size = 40) + geom_point(size=7,aes(attnmvec,BW_R2VecEst_ageless)) + ggtitle('EF Effect by PG3: Main Effect')+guides(alpha=FALSE,color=guide_legend(title="Maximal Y7 Overlap"))+theme(legend.position="top")
...
for (i in 1:length(colnames(individ_scalebybw_df))){
    ...
    EF_Bvec[i]<-EFEstVec(masterdf[,colindex])
    EF_BPvec[i]<-EFEstVecP(masterdf[,colindex])
...
## merged vectors
BwAgeCorTMDifDf<-data.frame(tmdifvec,ageEfvec,noAge_EFEfVec,EFEfvec,scalevec,Net1Vec,Net2Vec,Net1Vec17,Net2Vec17,fdr_ageEfPvec,fdr_EFEfPvec,fdr_noAge_EFEfPVec,ageDR2vec,occdifvec,all3pgDifvec)
# fdr age dr2
...
OccdifEF17<-ggplot(BwAgeCorTMDifDf,aes(x=occdifvec,y=noAge_EFEfVec,color=Net1Vec17)) + geom_text(size=5,label="\u25D6",family="Arial Unicode MS") + xlab('Occipality Difference') + ylab('EF Partial Correlation') + ggtitle('Pairwise Between-Network EF-FC Relations')+scale_color_manual(values=y17colors,limits=y17vec)+geom_text(aes(x=occdifvec,y=noAge_EFEfVec,color=Net2Vec17),size=5,label="\u25D7",family="Arial Unicode MS")+theme_classic()+ stat_smooth(method = "gam", formula = y ~ s(x), size = 1)
