...
% for i = 1:length(LeftCell)
	...
	% RESTING STATE JOB SUBMISSION
		...
		cmd = ['echo ' RStsFP_l ' >> ' sbjListFile];
        	system(cmd);
        	cmd = ['echo ' RStsFP_r ' >> ' sbjListFile];
        	system(cmd);
	...
	% NBACK JOB SUBMISSION
		 % set output FileName
                ResultantFile = [ResultantFolder_I '/IndividualParcel_Final_sbj1_comp' num2str(K) '_alphaS21_1_alphaL10_vxInfo1_ard0_eta0/final_UV_nback.mat'];
                % keep subject list file fresh
                sbjListFile = [ResultantFolder_I '/sbjListAllFile_' num2str(i) '.txt'];
                system(['rm ' sbjListFile]);
                % Insert filepaths to subject list file
                cmd = ['echo ' NBtsFP_l ' >> ' sbjListFile];
                system(cmd);
                cmd = ['echo ' NBtsFP_r ' >> ' sbjListFile];
                system(cmd);
                % save mat file for individualized NMF job
                save([ResultantFolder_I '/Configuration.mat'], 'sbjListFile', 'surfML', 'surfMR', 'PrepDataFile', 'ResultantFolder_I', 'resId', 'initName', 'K', 'alphaS21', 'alphaL', 'vxI', 'spaR', 'ard', 'eta', 'iterNum', 'calcGrp', 'parforOn');
                % Set script path
	...
	% EMO ID JOB SUBMISSION
		% set output FileName
                ResultantFile = [ResultantFolder_I '/IndividualParcel_Final_sbj1_comp' num2str(K) '_alphaS21_1_alphaL10_vxInfo1_ard0_eta0/final_UV_emoID.mat'];
                % keep subject list file fresh
                sbjListFile = [ResultantFolder_I '/sbjListAllFile_' num2str(i) '.txt'];
                system(['rm ' sbjListFile]);
                % Insert filepaths to subject list file
