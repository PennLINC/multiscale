...
# for a few different train and test splits
	...
	# get predicted EF values (ON TRAINING AS PROOF OF CONCEPT)
	predEF_AD=lm_AD.predict(xtrain_AD)
	predEF_AI=lm_AI.predict(xtrain_AI)
	predEF_ADL=lm_ADL.predict(xtrain_AD)
	predEF_AIL=lm_AIL.predict(xtrain_AI)
	# add predicted EF to indices this iteration was not trained on, add another number 
	subject_preds_AD[indices_train_AD,0]=subject_preds_AD[indices_train_AD,0]+predEF_AD
	subject_preds_AI[indices_train_AI,0]=subject_preds_AI[indices_train_AI,0]+predEF_AI
	subject_preds_AD[indices_train_AD,1]=subject_preds_AD[indices_train_AD,1]+1
	subject_preds_AI[indices_train_AI,1]=subject_preds_AI[indices_train_AI,1]+1
	subject_preds_ADL[indices_train_AD,0]=subject_preds_ADL[indices_train_AD,0]+predEF_ADL
	subject_preds_AIL[indices_train_AI,0]=subject_preds_AIL[indices_train_AI,0]+predEF_AIL
	subject_preds_ADL[indices_train_AD,1]=subject_preds_ADL[indices_train_AD,1]+1
	subject_preds_AIL[indices_train_AI,1]=subject_preds_AIL[indices_train_AI,1]+1
	# test prediction on TRAINING SET AS PROOF OF CONCEPT
	pred_obs_r2_AD = sklearn.linear_model.Ridge(alpha=alpha_AD).fit(xtrain_AD,ytrain_AD).score(xtrain_AD,ytrain_AD)
	pred_obs_r2_AI = sklearn.linear_model.Ridge(alpha=alpha_AI).fit(xtrain_AI,ytrain_AI).score(xtrain_AI,ytrain_AI)
	pred_obs_r2_ADL = sklearn.linear_model.Lasso(alpha=alpha_ADL).fit(xtrain_AD,ytrain_AD).score(xtrain_AD,ytrain_AD)
	pred_obs_r2_AIL = sklearn.linear_model.Lasso(alpha=alpha_AIL).fit(xtrain_AI,ytrain_AI).score(xtrain_AI,ytrain_AI)
...
#summary_topo_preds_permut[K-2,1]=mean_preds_permut
print("Unpermuted in-sample prediction - age Dep.:" + str(mean_preds_AD))
print("Unpermuted in-sample prediction - age Indep.:" + str(mean_preds_AI))
print("Unpermuted in-sample prediction - age Dep. Lasso:" + str(mean_preds_ADL))
print("Unpermuted in-sample prediction - age Indep. Lasso:" + str(mean_preds_AIL))
