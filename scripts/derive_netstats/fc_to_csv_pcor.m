...
bts_file=load('/cbica/projects/pinesParcels/results/aggregated_data/ind_conmats_allscales_allsubjs_pcor.mat');
bts_feats=bts_file.bTSP_indmats;
...
% get length of each, make an empty array of those combined +1 for subj ids

full_df_colnum=((length(win_over_scales)+length(Networkwise_seg_over_scales)+length(glob_seg_over_scales)+length(bw_over_scales)))+1;
...
third_df=cell(length(subjs)+1,full_df_colnum,1);
...
writetable(cell2table(df),strcat(outdir,'/master_fcfeats_pcor.csv'));
