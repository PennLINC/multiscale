...
for (i in 2:30){
  ...
  # annoying fucking exception for K=2's prissy ass not being able to figure out colMeans
  if (i==2){
    CenteredEdges<-masterdf[,scaleind]-mean(masterdf[,scaleind])
  } else {
    CenteredEdges<-masterdf[,scaleind]-colMeans(masterdf[,scaleind])
  }
  # center em so we can fit ridge without intercepts
  CenteredAge<-masterdf$Age-mean(masterdf$Age)
  forRRfc<-cbind(CenteredAge,masterdf$Motion,CenteredEdges)
...
forRRfc<-cbind(CenteredAge,masterdf$Motion,(masterdf[,scaleind]-colMeans(masterdf[,scaleind])))
...
edges<-masteref[,scaleK_bw_indivi_cols_inds]
  # annoying fucking exception for K=2's prissy ass not being able to figure out colMeans
  if (i==2){
    centeredEdges<-edges-mean(edges)
  } else {
    centeredEdges<-edges-colMeans(edges)
  }
# center em so we can fit ridge without intercepts
centeredEdges<-edges-colMeans(edges)
centeredEF<-masteref$F1_Exec_Comp_Cog_Accuracy-mean(masteref$F1_Exec_Comp_Cog_Accuracy)
# combine feature matrices with EF scores
...
edges_EF<-cbind((alledges-colMeans(alledges)),masteref$F1_Exec_Comp_Cog_Accuracy)
