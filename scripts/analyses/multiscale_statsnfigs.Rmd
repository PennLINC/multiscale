...
avgrecon_softParVar<-ggplot(data=mcrecnerrdf,aes(y=mreconerr,x=community_vec))+geom_step(size=3,color='#0375B4') +geom_step(aes(y=SoftVar*2e6,x=community_vec),size=3,color='#262228') +ggtitle("Reconstruction Error Over Scales")+ scale_x_continuous(name ="# of Communitites",  breaks=seq(2,30,4))+scale_y_continuous(sec.axis=sec_axis(trans =as.formula(~./2e6),name = 'Median Absolute Deviation'))+labs(y='Recon Error')+theme_minimal(base_size = 25)+ theme(axis.title.y.left =  element_text(color = '#0375B4'),axis.text.y.left =  element_text(color = '#0375B4'))
