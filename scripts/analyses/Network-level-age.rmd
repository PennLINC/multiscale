...
summary(SM_lm)
summary(DM_lm)
# convert yeo7 membership to vector capturing only sig. yeo17 networks, graying out nonsig.
domnetSig<-domnetvec
levels(domnetSig)<-c(levels(domnetSig),'zNonSig_DM','zNonSig_Mot')
bwdf$domnetvecSig<-as.character(domnetvec)
# set nonsigs to de-saturated version of colors
bwdf$domnetvecSig[NL_sigVec==FALSE & bwdf$domnetvecSig == 'Motor']='zNonSig_Mot'
bwdf$domnetvecSig[NL_sigVec==FALSE & bwdf$domnetvecSig == 'DM']='zNonSig_DM'


# use bootstraps to determine signifiance

# for linear Motor A Age Effect ~ Scale - significance
CI_LIN=quantile(bootstrapRDS$SMA_testStatLin,c(0.025,0.975)) 

# discrete p calculation (https://www.bmj.com/content/343/bmj.d2304 as source)
SE=(CI_LIN[2]-CI_LIN[1])/(2*1.96)
z=SMA_lm$coefficients['poly(scalesvec, 2)1']/SE
z=abs(z)
pLIN<-exp((-0.717*z)-(0.416*(z^2)))
print(CI_LIN)
print(pLIN)

# for Quadratic Motor Age Effect ~ Scale - significance
CI_QUADR=quantile(bootstrapRDS$SMA_testStatQuadr,c(0.025,0.975)) 

# discrete p calculation (https://www.bmj.com/content/343/bmj.d2304 as source)
SE=(CI_QUADR[2]-CI_QUADR[1])/(2*1.96)
z=SMA_lm$coefficients['poly(scalesvec, 2)2']/SE
z=abs(z)
...
# DMB for comparison
# for linear DMB Age Effect ~ Scale - significance
CI_LIN=quantile(bootstrapRDS$DMB_testStatLin,c(0.025,0.975)) 

# discrete p calculation (https://www.bmj.com/content/343/bmj.d2304 as source)
SE=(CI_LIN[2]-CI_LIN[1])/(2*1.96)
z=DMB_lm$coefficients['poly(scalesvec, 2)1']/SE
z=abs(z)
pLIN<-exp((-0.717*z)-(0.416*(z^2)))
print(CI_LIN)
print(pLIN)
...
# for Quadratic Motor Age Effect ~ Scale - significance
CI_QUADR=quantile(bootstrapRDS$DMB_testStatQuadr,c(0.025,0.975)) 

# discrete p calculation (https://www.bmj.com/content/343/bmj.d2304 as source)
SE=(CI_QUADR[2]-CI_QUADR[1])/(2*1.96)
z=DMB_lm$coefficients['poly(scalesvec, 2)2']/SE
z=abs(z)
pQUADR<-exp((-0.717*z)-(0.416*(z^2)))
print(CI_QUADR)
print(pQUADR)
...
geom_smooth(data=subset(bwdf,domnetvec17=='Somatomotor A'),method='lm',formula = y~poly(x,2),aes(color=domnetvec17),fill="gray72",size=2)+geom_smooth(data=subset(bwdf,domnetvec17=='DM_B'),size=2,method='lm',formula = y~poly(x,2),aes(color=domnetvec17),fill="gray88")+scale_color_manual(values=c('#bc0943','#4183a8'))+geom_point(data=subset(bwdf,domnetvec17=='Somatomotor A'),aes(color=domnetvec17Sig),size=5)+geom_point(data=subset(bwdf,domnetvec17=='DM_B'),aes(color=domnetvec17Sig),size=5)+scale_color_manual(values=c('#bc0943','#4183a8','#ebbecc','#b7d9ed'),labels=c('DM B','SM A','n.s. DM B','n.s. SM A'))+scale_x_continuous(breaks=c(4,10,16,22,28))+theme(plot.margin=unit(c(.9,.6,2,.6),"cm"))


ggplot(bwdf,aes(scalesvec,avg_bw_deltaR2)) + xlab("# of Networks") + ylab(expression(paste('Age Effect (',Delta,R^2[adj],')'))) +theme_classic(base_size = 28) +guides(alpha=FALSE,color=guide_legend(title="Yeo 17 Overlap"))+theme(legend.position=c(.39,-.19),legend.direction = "horizontal",legend.text = element_text(size=20),legend.title = element_text(size=24))+
    geom_smooth(data=subset(bwdf,domnetvec=='Motor'),method='lm',formula = y~poly(x,2),aes(color=domnetvec),fill="gray72")+geom_smooth(data=subset(bwdf,domnetvec=='DM'),method='lm',formula = y~poly(x,2),aes(color=domnetvec),fill="gray88")+scale_color_manual(values=c('#bc0943','#4183a8'))+geom_point(data=subset(bwdf,domnetvec=='Motor'),aes(color=domnetvecSig),size=5)+geom_point(data=subset(bwdf,domnetvec=='DM'),aes(color=domnetvecSig),size=5)+scale_color_manual(values=c('#bc0943','#4183a8','#ebbecc','#b7d9ed'),labels=c('DM','SM','n.s. DM','n.s. SM'))+scale_x_continuous(breaks=c(4,10,16,22,28))+theme(plot.margin=unit(c(.9,.6,2,.6),"cm"))
