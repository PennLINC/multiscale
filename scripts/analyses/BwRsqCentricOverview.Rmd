...
for (i in 1:29){
  ...
  bootparcor<-boot(Bwscaledf,returnCor,R=10000)
...
netLM<-lm(BW_R2VecEst~scalesvec*occvec,bwR2EFdf)
...
to_be_melteddf<-data.frame(cbind(indglobBw_EF$EF,indglobBw_EF$Age,indglobBw_EF$Sex,indglobBw_EF$Motion,indglobBw_EF[,6:34]))
melteddf<-melt(to_be_melteddf,id=c('indglobBw_EF.EF','indglobBw_EF.Age','indglobBw_EF.Sex','indglobBw_EF.Motion'))
# replace x2...x30 with appropriate length scalesvec
scalesvec_20097<-rep(seq(2:30),each=693)
melteddf$variable<-log(scalesvec_20097)
a<-lm(value~indglobBw_EF.EF+indglobBw_EF.Sex+variable*indglobBw_EF.Age,melteddf)
plot_model(a,type='int')
