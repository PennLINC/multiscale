...
for (i in 1:29){
  ...
  print(ggplot(indglobWin_motSexC,aes(Age,indglobWin_motSexC[,i+4])) + geom_point() + geom_smooth() +ylab(paste("Ind Global Win at Scale", i+1)))
    print(ggplot(indglobBw_motSexC,aes(Age,indglobBw_motSexC[,i+4])) + geom_point(color='#AAAAAA',alpha=.5) + geom_smooth(method='gam',formula=y~s(x,k=3),color='black') +ylab(paste("B/w at Scale", i+1))+theme_classic())
