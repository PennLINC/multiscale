...
fig5v2<-ggplot(bwdf,aes(scalesvec,bwAgeCorVecs.avg_bw,color=domnetvec17)) + xlab("# of Communities") + ylab(expression(paste('Age Effect (',Delta,R^2[adj],')'))) +theme_classic(base_size = 28) +guides(alpha=FALSE,color=guide_legend(title="Yeo 17 Overlap"))+theme(legend.position='top',legend.text = element_text(size=20),legend.title = element_text(size=24))+geom_smooth(data=subset(bwdf,domnetvec17=='Somatomotor A'),method='gam',formula = y~s(x,k=3))+geom_smooth(data=subset(bwdf,domnetvec17=='DM_B'),method='gam',formula = y~s(x,k=3))+geom_point(data=subset(bwdf,domnetvec17=='Somatomotor A'),size=4)+geom_point(data=subset(bwdf,domnetvec17=='DM_B'),size=4)+scale_color_manual(values=c('#bc0943','#4183a8'),labels=c('Default Mode B','Somatomotor A'))+scale_x_continuous(breaks=c(4,10,16,22,28))
...
Somat_A_bwdf<-bwdf[domnetvec17=='Somatomotor A',]
DM_B_bwdf<-bwdf[domnetvec17=='DM_B',]
# for underscoreless label
bwdf$domnetvec17='DM B'
fig5Cv2<-ggplot(bwdf,aes(scalesvec,bwAgeCorVecs.avg_bw,color=domnetvec17)) + xlab("# of Communities") + ylab(expression(paste('Age Effect (',Delta,R^2[adj],')'))) +theme_classic(base_size = 28) +guides(alpha=FALSE,color=guide_legend(title="Maximal Y7 Overlap"))+theme(legend.position='top')+geom_smooth(data=subset(bwdf,domnetvec17=='Somatomotor A'),method='gam',formula = y~s(x,k=4))+geom_smooth(data=subset(bwdf,domnetvec17=='DM_B'))+geom_point(data=subset(bwdf,domnetvec17=='Somatomotor A'),size=3)+geom_point(data=subset(bwdf,domnetvec17=='DM_B'),size=3)+scale_color_manual(values=c('#bc0943','#4183a8'))
...
fig6v2<-ggplot(bwR2EFdf,aes(scalesvec,BW_R2VecEst,color=domnetvec17)) + xlab("# of Communities") + ylab(expression(paste('EF Effect (',Delta,R^2[adj],')'))) +theme_classic(base_size = 28) +guides(alpha=FALSE,color=guide_legend(title="Yeo 17 Overlap"))+theme(legend.position='top',legend.text = element_text(size=20),legend.title = element_text(size=24))+geom_smooth(data=subset(bwR2EFdf,domnetvec17=='Somatomotor A'),method='gam',formula = y~s(x,k=3))+geom_smooth(data=subset(bwR2EFdf,domnetvec17=='DM_B'),method='gam',formula = y~s(x,k=3))+geom_point(data=subset(bwR2EFdf,domnetvec17=='Somatomotor A'),size=4)+geom_point(data=subset(bwR2EFdf,domnetvec17=='DM_B'),size=4)+scale_color_manual(values=c('#bc0943','#4183a8'),labels=c('Default Mode B','Somatomotor A'))+scale_x_continuous(breaks=c(4,10,16,22,28))
...
gg_tensor(g2)+theme_classic(base_size = 40) +theme(legend.text=element_text(size=30),legend.key.width =unit(3.5,"cm"),legend.title = element_text(size=30))+scale_fill_gradient2(low = "blue",high = "red",mid = "white",midpoint = 0,name=expression(paste('Age Effect(',Delta,R^2[adj],')')),guide = guide_colorbar(legend.position='top',title.position="top",ticks.colour = "gray50", ticks.linewidth = 7))+ggtitle("")+xlab("Community Transmodality")+ylab("Community Transmodality")+geom_vline(xintercept = mean(range(doubleBwAgeCorTMDifDf$net1tmvec)),linetype='dashed',size=1.5)+geom_hline(yintercept = mean(range(doubleBwAgeCorTMDifDf$net1tmvec)),linetype='dashed',size=1.5)+theme(legend.position='top')
