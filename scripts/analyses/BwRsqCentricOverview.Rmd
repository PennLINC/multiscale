...
# for figure 4
ggplot(BwAgeCorTMDifDf[BwAgeCorTMDifDf$ageDR2Pvec<0.05,],aes(x=tmdifvec,y=ageDR2vec,color=Net1Vec))+ geom_text(size=7,label="\u25D6",family="Arial Unicode MS")+scale_color_manual(values=ycolors,limits=Y7vec)+geom_text(aes(x=tmdifvec,y=ageDR2vec,color=Net2Vec),size=7,label="\u25D7",family="Arial Unicode MS")+theme(legend.position = "none") + xlab('Gradient Distance') + ylab(expression(paste(Delta,R^2[adj])))+theme_classic(base_size = 35)+theme(legend.position = "none")
...
```{r}
######### Adapted for K=17 vs. Multiscale features
for (k in 2:30){
# grep out edges at this scale
scaleStr=paste('scale',k,'_',sep='')
scaleCols_inds=grep(scaleStr,colnames(masteref))
