...
```{r include=FALSE}
### it's rounded now bb
fc<-vroom('/cbica/projects/pinesParcels/results/aggregated_data/fc/master_fcfeats_pcor.csv',col_names = T)
colnames(fc)<-fc[1,]
...
# make them numeric
masterdf[,basists]<-as.numeric(unlist(masterdf[,basists]))
```
...
hist(occvec,12,xlab="Occipitality",ylab="Count",ylim=c(0,70), main=NULL,col="grey")
hist(attnmvec,12,xlab="Attentionality",ylab="Count",ylim=c(0,70), main=NULL,col="grey")
...
indiv_bwcols_ind<-intersect(bwcol,basists)
individ_scalebybw_df<-masterdf[,indiv_bwcols_ind]
indiv_wincols_ind<-intersect(wincols,basists)
...
# avg b/w
# zeroing line
bwdf$bwAgeCorVecs.avg_bw[bwdf$bwAgeCorVecs.avg_bwP>0.05]<-0
#
...
unibw_age_scale<-ggplot(bwdf,aes(tmvec,bwAgeCorVecs.bw_to_unimodal,color=scalesvec)) + geom_point(size=6) + xlab("Transmodality") + ylab("AgeBWUCor")+theme_classic(base_size = 28)+ ylim(-.1,.1)
####
...
library(plot3Drgl)
bwdf$occvec<-occvec
bwdf$attnmvec<-attnmvec
bwdf$tmvec<-tmvec
palette(c('#007500','#c1253c','#d77d00','#b8cf86','#3281ab','#b61ad0','#670068'))
plot3d(x=bwdf$tmvec,y=bwdf$occvec,z=bwdf$attnmvec,size = 12,col=as.integer(bwdf$domnetvec))
palette(colorRampPalette(c("yellow","black"))(464))
#### section still just reflects pcor
# rank age effects for coloring - can copy this to prev. age effect quant
plot3d(x=bwdf$tmvec,y=bwdf$occvec,z=bwdf$attnmvec,size = 12,col=as.integer(rank(bwdf$bwAgeCorVecs.avg_bw)))
```
...
InflectionEst<-rep(0,length=length(masterdf[,indiv_nsegcols_ind]))
...
for (i in 1:(length(bwAvgCondf)-3)){
  ...
  InflectionEst[i]<-grep(min(abs(derv$derivative)),derv$derivative)
...
AgeSpan_plotdf2<-data.frame(tmvec,scalesvec,domnetvec,corVecEst,netpropvec,CIgroupingInd,InflectionEst)
...
BwspanPcorColor<-ggplot(LongAgeSpan_plotdf2,aes(agespans,tmvec,color=derivs,group=CIgroupingInd)) +geom_line(size=10) +labs(title='External Connectivity Effect Span', x = 'Age', y = 'Transmodality') +theme_classic(base_size = 58)+ xlim(c(8,23))+ scale_colour_gradientn(colours=c('#0b0bff','#8585ff','white','#ff9191','#ff0a0a'),values = c(0,.25,.5,.75,1),breaks=breaks, labels = breaks,limits=c(-.02,0.02),name="Change Per Year")+theme(legend.position="top",legend.key.width = unit(5, "cm"))+ylim(c(min(tmvec),max(tmvec)))
...
# Inflection estimates
plot(InflectionEst,tmvec)
plot(InflectionEst[InflectionEst!=0],tmvec[InflectionEst!=0])

y7veInflecr<-ggplot(LongAgeSpan_plotdf2,aes(x=InflectionEst,y=tmvec,group=CIgroupingInd)) +geom_point(size=10) +labs(title='External Connectivity Effect Span', x = 'Age', y = 'Transmodality') +theme_classic(base_size = 58)+ scale_color_manual(values=c('#007500','#c1253c','#d77d00','#b8cf86','#3281ab','#b61ad0','#670068'),name="Maximal Y7 Overlap")+theme(legend.position="top",legend.key.width = unit(5, "cm"))+ylim(c(min(tmvec),max(tmvec)))
```
...
for (i in 1:length(colnames(individ_scalebybw_df))){
    ...
    tmdif=abs(tm1-tm2)
    occdif=abs(oc1-oc2)
    # exper. 3-grad distance calc
    pg3dif=abs(tm1-tm2)+abs(oc1-oc2)+abs(atn1-atn2)
    ...
    pcordf<-cbind(as.numeric(masterdf$Age),as.numeric(masterdf$Sex),masterdf$Motion,masterdf[,colindex])
    ...
    EFpcordf<-cbind(masteref$F1_Exec_Comp_Cog_Accuracy,as.numeric(masteref$Age),as.numeric(masteref$Sex),masteref$Motion,masterdf[,colindex])
    # get EF relation controlling for sex and motion
    EFpcordf2<-cbind(masteref$F1_Exec_Comp_Cog_Accuracy,as.numeric(masteref$Sex),masteref$Motion,masteref[,colindex])
...
## merged vectors
BwAgeCorTMDifDf<-data.frame(tmdifvec,ageEfvec,noAge_EFEfVec,EFEfvec,scalevec,Net1Vec,Net2Vec,Net1Vec17,Net2Vec17,fdr_ageEfPvec,fdr_EFEfPvec,fdr_noAge_EFEfPVec,all3pgDifvec)
...
ggplot(data = BwAgeCorTMDifDf, aes(x = factor(scalevec), y = all3pgDifvec)) +
  geom_violin(alpha = .8) +
...
for (K in 2:30){
  ...
  # annoying additional logical to deal with single columns running into rowmeans
  if (length(indiv_bwcols_ind[AgeAli_K])==1){
    winScaleAvgbwVal_agF[,K-1]<-mean(masterdf[,indiv_bwcols_ind[AgeAli_K]])
  } else {
...
for (i in 1:29){
  # relevant df
  scaledf<-cbind(as.numeric(indglobbw$EF),as.numeric(indglobbw$Age),as.numeric(indglobbw$Sex),as.numeric(indglobbw$Motion),as.numeric(indglobbw[,i+5]))
  # partial spearmans to extrac age relation
  pspear=pcor(scaledf,method='spearman')$estimate
...
for (i in 1:29){
  ...
  scaledf<-cbind(as.numeric(indglobbw$EF),as.numeric(indglobbw$Sex),as.numeric(indglobbw$Motion),as.numeric(indglobbw[,i+5]))
  # partial spearmans to extrac age relation
  pspear=pcor(scaledf,method='spearman')$estimate
...
### age mitigated
indglobbw<-data.frame(cbind(as.numeric(masteref$bblid),as.numeric(masteref$Age/12),as.numeric(masteref$Sex),as.numeric(masteref$Motion),as.numeric(masteref$F1_Exec_Comp_Cog_Accuracy),as.numeric(winScaleAvgbwVal_agM)))
colnames(indglobbw)[1:5]<-c('bblid','Age','Sex','Motion','EF')
ind_bwcors<-matrix(0,29,2)
ind_bwcors[,1]<-2:30
for (i in 1:29){
...
# just for global age
indglobbw<-data.frame(cbind(as.numeric(masteref$bblid),as.numeric(masteref$Age/12),as.numeric(masteref$Sex),as.numeric(masteref$Motion),as.numeric(winScaleAvgbwVal)))
colnames(indglobbw)[1:4]<-c('bblid','Age','Sex','Motion')
ind_bwcors<-matrix(0,29,2)
ind_bwcors[,1]<-2:30
for (i in 1:29){
  # relevant df
  scaledf<-cbind(as.numeric(indglobbw$Age),as.numeric(indglobbw$Sex),as.numeric(indglobbw$Motion),as.numeric(indglobbw[,i+4]))
  # partial spearmans to extrac age relation
  pspear=pcor(scaledf,method='spearman')$estimate
  ind_bwcors[i,2]<-pspear[4]
}
...
######### do age subsets
##8-13
#indglobbw<-data.frame(cbind(masteref$bblid,masteref$Age/12,masteref$Sex,mast#eref$Motion,winScaleAvgbwVal))
#colnames(indglobbw)[1:4]<-c('bblid','Age','Sex','Motion')
...
indiv=grep(basists,colnames(masteref))
...
# same M - edges
write.table(edges,'/cbica/projects/pinesParcels/data/aggregated_data/Mpcor.csv',sep=',',row.names = F,col.names = F)
```
