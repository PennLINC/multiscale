...
CI_LIN=quantile(bootstrapRDS$lm_testStatQUADR,c(0.025,0.975)) 
...
################# For Network by scale effects


SMA_lm<-lm(EFDR2vec~poly(scalesvec,2),data=bwdf[bwdf$domnetvec17=='Somatomotor A',])
DMB_lm<-lm(EFDR2vec~poly(scalesvec,2),data=bwdf[bwdf$domnetvec17=='DM_B',])
...
geom_smooth(data=subset(bwdf,domnetvec17=='Somatomotor A'),method='lm',formula = y~poly(x,2),aes(color=domnetvec17),fill="gray72")+geom_smooth(data=subset(bwdf,domnetvec17=='DM_B'),method='lm',formula = y~poly(x,2),aes(color=domnetvec17),fill="gray88")+scale_color_manual(values=c('#bc0943','#4183a8'))+geom_point(data=subset(bwdf,domnetvec17=='Somatomotor A'),aes(color=domnetvec17Sig),size=4)+geom_point(data=subset(bwdf,domnetvec17=='DM_B'),aes(color=domnetvec17Sig),size=4)+scale_color_manual(values=c('#bc0943','#4183a8','#ebbecc','#b7d9ed'),labels=c('DM B','SM A','n.s. DM B','n.s. SM A'))+scale_x_continuous(breaks=c(4,10,16,22,28))+theme(plot.margin=unit(c(.9,.6,2,.6),"cm"))
```