...
CIplotdf<-data.frame(rbind(confveclower_d,confvecupper_d))
# range of interest until we work out K=2 and K=3
indofint<-6:464
indofint<-append(indofint,(470:928))

# plot it
Win_age_tm<-ggplot(CIplotdf[indofint,],aes(tmvec,CIbounds,color=domnetvec,alpha=netpropvec^2,group=V7)) + geom_line(size=1) + scale_color_manual(values=c('#007500','#c1253c','#d77d00','#b8cf86','#3281ab','#b61ad0','#670068')) + xlab("Transmodality") + ylab("AgeWithinCor") +theme_classic() + geom_point(size=4,aes(tmvec,corVecEst)) + ggtitle('Correlation of Within Network Connectivity and Age over All Networks')

# check for scale-specific effects
Win_age_scale<-ggplot(CIplotdf[indofint,],aes(tmvec,corVecEst,color=scalesvec)) + geom_point() + xlab("Transmodality") + ylab("AgeSegregCor")
```
...
Segreg_age_tm<-ggplot(CIplotdf[indofint,],aes(tmvec,CIbounds,color=domnetvec,alpha=netpropvec^2,group=V7)) + geom_line(size=1) + scale_color_manual(values=c('#007500','#c1253c','#d77d00','#b8cf86','#3281ab','#b61ad0','#670068')) + xlab("Transmodality") + ylab("AgeSegregCor") +theme_classic() + geom_point(size=4,aes(tmvec,corVecEst)) + ggtitle('Correlation of Network Segregation and Age over All Networks')

Segreg_age_scale<-ggplot(CIplotdf[indofint,],aes(tmvec,corVecEst,color=scalesvec)) + geom_point() + xlab("Transmodality") + ylab("AgeSegregCor")
...
# empty array to populate with b/w connectivity age cors (b/w to unimodal, b/w to transmodal, b/w to aggregate, and K and N just to confirm we are matching)
bwAgeCorVecs<-matrix(0,464,5)
...
for (i in 1:2){
  ...
  for (K in 2:30){
    ...
    for (N in 1:K){
      ...
      Ncolname<-grep(as.character(N),bwnetnames_thisScale,value=T)
      ...
      Ncolname<-paste('_',Ncolname,'_',sep='')
      ...
      matchTruncColName<-sub('.','',matchTruncColName)
      match_NetN_scaleK_bw_indivi_cols_names<-grep(as.character(paste(matchTruncColName,collapse="|")),bwcolnames_thisScale,value=T)
      # deep-sea grepping the whole paste and pipe thing is just to deal with character vectors instead of single patterns
      match_NetN_scaleK_bw_indivi_cols_ind<-grep(as.character(paste(match_NetN_scaleK_bw_indivi_cols_names,collapse="|")),colnames(masterdf))
      ...
      unmatchTruncColName<-sub('.','',unmatchTruncColName)
      unmatch_NetN_scaleK_bw_indivi_cols_names<-grep(as.character(paste(unmatchTruncColName,collapse="|")),bwcolnames_thisScale,value=T)
      # deep-sea grepping the whole paste and pipe thing is just to deal with character vectors instead of single patterns
      unmatch_NetN_scaleK_bw_indivi_cols_ind<-grep(as.character(paste(unmatch_NetN_scaleK_bw_indivi_cols_names,collapse="|")),colnames(masterdf))
    ...
    # get average bw network connectivty age correlation for this network at this scale
      ...
      bwAgeCorVecs[Nind,3]=unlist(avg_bw_agecor)
    }
...
# ensure 464 length (number of scale by net features)
length(indiv_scalebynet_df)
avgbwdf<-data.frame(tmvec,scalesvec,domnetvec,netpropvec,bwAgeCorVecs$avg_bw)
...
# avg b/w
avgbw_age_tm<-ggplot(bwdf[indofint,],aes(tmvec,bwAgeCorVecs.avg_bw,color=domnetvec,alpha=netpropvec^2)) + geom_point()+ scale_color_manual(values=c('#007500','#c1253c','#d77d00','#b8cf86','#3281ab','#b61ad0','#670068')) + xlab("Transmodality") + ylab("AgeBWCor") +theme_classic() + ggtitle('Correlation of Network B/w con avg and Age over All Networks')

avgbw_age_scale<-ggplot(avgbwdf[indofint,],aes(tmvec,bwAgeCorVecs.avg_bw,color=scalesvec)) + geom_point() + xlab("Transmodality") + ylab("AgeBWCor")

# b/w to unimodal
unibw_age_tm<-ggplot(bwdf[indofint,],aes(tmvec,bwAgeCorVecs.bw_to_unimodal,color=domnetvec,alpha=netpropvec^2)) + geom_point(size=3)+ scale_color_manual(values=c('#007500','#c1253c','#d77d00','#b8cf86','#3281ab','#b61ad0','#670068')) + xlab("Transmodality") + ylab("AgeBWUCor") +theme_classic() + ggtitle('Correlation of Network B/w unimodal con and Age over All Networks')

unibw_age_scale<-ggplot(bwdf[indofint,],aes(tmvec,bwAgeCorVecs.bw_to_unimodal,color=scalesvec)) + geom_point() + xlab("Transmodality") + ylab("AgeBWUCor")

# b/w to transmodal
transbw_age_tm<-ggplot(bwdf[indofint,],aes(tmvec,bwAgeCorVecs.bw_to_transmodal,color=domnetvec,alpha=netpropvec^2)) + geom_point()+ scale_color_manual(values=c('#007500','#c1253c','#d77d00','#b8cf86','#3281ab','#b61ad0','#670068')) + xlab("Transmodality") + ylab("AgeBWTCor") +theme_classic() + ggtitle('Correlation of Network B/w transmodal con and Age over All Networks')

transbw_age_scale<-ggplot(bwdf[indofint,],aes(tmvec,bwAgeCorVecs.bw_to_transmodal,color=scalesvec)) + geom_point() + xlab("Transmodality") + ylab("AgeBWTCor")

ggarrange(avgbw_age_tm,avgbw_age_scale,unibw_age_tm,unibw_age_scale,)
```
```
