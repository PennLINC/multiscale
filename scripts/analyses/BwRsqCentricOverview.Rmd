...
fig6v2<-ggplot(bwR2EFdf,aes(scalesvec,BW_R2VecEst,color=domnetvec17)) + xlab("# of Communities") + ylab(expression(paste('EF Effect (',Delta,R^2[adj],')'))) +theme_classic(base_size = 28) +guides(alpha=FALSE,color=guide_legend(title="Maximal Yeo Overlap"))+theme(legend.position='top')+geom_smooth(data=subset(bwR2EFdf,domnetvec17=='Somatomotor A'),method='gam',formula = y~s(x,k=3))+geom_smooth(data=subset(bwR2EFdf,domnetvec17=='DM_B'),method='gam',formula = y~s(x,k=3))+geom_point(data=subset(bwR2EFdf,domnetvec17=='Somatomotor A'),size=3)+geom_point(data=subset(bwR2EFdf,domnetvec17=='DM_B'),size=3)
...
Bwspan<-ggplot(LongAgeSpan_plotdf2[derivSigWholeSpline,],aes(agespans,tmvecJit,color=derivs,group=CIgroupingInd,alpha=AlphaSig)) +geom_line(size=3) +labs(x = 'Age', y = 'Transmodality') +theme_classic(base_size = 40)+ xlim(c(8,23))+ scale_colour_gradientn(colours=c('#0b0bff','#8585ff','white','#ff9191','#ff0a0a'),values = c(0,.4,.5,.6,1),breaks=breaks, labels = breaks,limits=c(-.0031,0.0031),name="Change Per Year (B/w Con.)")+theme(legend.text=element_text(size=30),legend.title = element_text(size=30),legend.position="top",legend.key.width = unit(3, "cm"))+ylim(c(min(tmvecJit),max(tmvecJit)))+guides(alpha=FALSE,color = guide_colorbar(title.position="top",ticks.colour = "gray",
                                ticks.linewidth = 8))+geom_vline(xintercept = c(10,15,20),linetype='dashed',size=1.5)+ theme(legend.margin=margin(b=-.5,t = -.5, unit='cm')) + scale_x_continuous(
    limits = c(8, 23),
    expand = c(0, 0),
...
for (K in 2:30){
      ...
      at16=BW_deriv15vec[Kind]
      at21=BW_deriv20vec[Kind]
      # write em out to cubic
