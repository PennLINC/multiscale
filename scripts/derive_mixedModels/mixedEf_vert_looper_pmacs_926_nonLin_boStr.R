...
returnLmCoefs<-function(d,i){
  ...
  d2<-d[i,]
  model=gamm(value~Motion+Sex+ti(Scale)+ti(Age)+ti(Scale,Age),random=list(bblid=~1),data=d2)
...
for (v in 17001:17734){
  ...
  model=gamm(value~Motion+Sex+ti(Scale)+ti(Age)+ti(Scale,Age),random=list(bblid=~1),data=mdf_verts)
  fe=fixef(model$lme)
  #re=ranef(model$lme)
  #re=re$bblidi
  ...
  Lresults <- boot(data=mdf_verts, statistic=returnLmCoefs, R=10000)
  ...
  # non linear coefs
  NLresults<-boot(data=mdf_verts,statistic=returnGammFs,R=10000)
  # index 1 is Scale
  ...
  write.table(outarray,file=paste('~/mixedEffectModels/Modeled_fSex_fMot_s_fAgexScale_raInt_v',v,'_bwVals_BoStr_overScales.csv',sep=''),sep=',',row.names=F,quote=F)
  # 9/10/20 section to write out subject level intercepts and slopes
