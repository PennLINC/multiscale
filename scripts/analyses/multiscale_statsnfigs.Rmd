...
# check if new fc selectively replaced K=3
...
#### USES RESIDUALS, NOT SIMULTANEOUSLY FITTED AGE COEFFICIENT AT THE MOMENT ### APPLIES TO LINES 178-300
for (i in 1:29){
  # i+5 because first column is scanid, second is age, third is sex, 4th is motion, 5th is EF
  print(ggplot(indglobseg,aes(Age,indglobseg[,i+5])) + geom_point() + geom_smooth() +ylab(paste("Ind Global Segreg at Scale", i+1)))
  linmod=lm(indglobseg[,i+5]~Sex+Motion,data=indglobseg)
  agecor=(cor.test(linmod$residuals,groglobseg$Age,method = 'spearman'))$estimate
...
gro_segcors[,1]<-2:30
for (i in 1:29){
  print(ggplot(groglobseg,aes(Age,groglobseg[,i+5])) + geom_point() + geom_smooth() +ylab(paste("Gro Global Segreg at Scale", i+1)))
  # i+5 because first column is scanid, second is age, third is sex, 4th is motion, 5th is EF
...
for (i in 1:29){
  # i+5 because first column is scanid, second is age, third is sex, 4th is motion, 5th is EF
   print(ggplot(indglobseg,aes(EF,indglobseg[,i+5])) + geom_point() + geom_smooth() +ylab(paste("Ind Global Segreg at Scale", i+1)))
  linmod=lm(indglobseg[,i+5]~Sex+Motion+Age,data=indglobseg)
  EFcor=(cor.test(linmod$residuals,indglobseg$EF,method='spearman'))$estimate
...
for (i in 1:29){
  # i+5 because first column is scanid, second is age, third is sex, 4th is motion, 5th is EF
  linmod=lm(groglobseg[,i+5]~Sex+Motion+Age+EF,data=groglobseg)
...
ggplot(Seg_CIplotdf[indofint,],aes(tmvec,Tot_patchnum))+ geom_pointdensity(size=4, adjust = 3)+ xlab("Transmodality") + ylab("Total Patch number")+ ggtitle('Correlation of Network Segregation and Age over All Networks')
```
