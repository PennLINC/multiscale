...
########### USE THESE AS MASKS FOR DEVELOPMENTALLY FACILITATED VS. CONFLICTED

## merged vectors
BwAgeCorTMDifDf<-data.frame(tmdifvec,ageEfvec,noAge_EFEfVec,scalevec,Net1Vec,Net2Vec,Net1Vec17,Net2Vec17)

ggplot(BwAgeCorTMDifDf,aes(x=tmdifvec,y=ageEfvec)) + geom_point(alpha=.07,size=5) + xlab('Transmodality Difference') + ylab('Age Partial Correlation') + ggtitle('Pairwise Between-Network Age-FC Relations')
...
ageAligned<-as.matrix(rep(999,dim(BwAgeCorTMDifDf)[1]))
ageMitigated<-as.matrix(rep(999,dim(BwAgeCorTMDifDf)[1]))
ageAligned_stringent<-as.matrix(rep(999,dim(BwAgeCorTMDifDf)[1]))
ageMitigated_stringent<-as.matrix(rep(999,dim(BwAgeCorTMDifDf)[1]))


for (feat in 1:dim(BwAgeCorTMDifDf)[1]){
  # if age and EF trend are aligned (- - or + +)
  ...
  } else if (BwAgeCorTMDifDf$ageEfvec[feat]*BwAgeCorTMDifDf$noAge_EFEfVec[feat] < 0) {
    # if age and EF trend are opposed
    ageAligned[feat]=0
    ageAligned_stringent[feat]=0
    ageMitigated[feat]=1
    ageMitigated_stringent[feat]=1
  }
  # if either effect size is smaller than .1, might be hard to concretely dissociate
  # extra threshold for stringent
  if (abs(BwAgeCorTMDifDf$ageEfvec[feat]) < .1 || abs(BwAgeCorTMDifDf$noAge_EFEfVec[feat]) < .1){
    ageMitigated_stringent[feat]=0
    ageAligned_stringent[feat]=0
  }
...
agM_y7<-ggplot(ageMitigateddf,aes(x=tmdifvec,y=noAge_EFEfVec,color=Net1Vec)) + geom_text(size=10,label="\u25D6",family="Arial Unicode MS") + xlab('Transmodality Difference') + ylab('EF Partial Correlation') + ggtitle('Pairwise Between-Network Age-mitigated EF-FC Relations')+scale_color_manual(values=ycolors,limits=Y7vec)+geom_text(aes(x=tmdifvec,y=noAge_EFEfVec,color=Net2Vec),size=10,label="\u25D7",family="Arial Unicode MS")+theme_classic()+xlim(0,12)+ylim(-.3,.3)
...
for (K in 2:30){
  ...
  AM_corvec[K-1]<-mean(AM_K$noAge_EFEfVec)
  both_corvec[K-1]<-mean(both$noAge_EFEfVec)
}
plot(AA_corvec)
plot(AM_corvec)
plot(both_corvec)
...
agM_y7_thresh<-ggplot(ageMitigateddf_stringent,aes(x=tmdifvec,y=noAge_EFEfVec,color=Net1Vec)) + geom_text(size=10,label="\u25D6",family="Arial Unicode MS") + xlab('Transmodality Difference') + ylab('EF Partial Correlation') + ggtitle('Pairwise Between-Network Age-mitigated EF-FC Relations')+scale_color_manual(values=ycolors,limits=Y7vec)+geom_text(aes(x=tmdifvec,y=noAge_EFEfVec,color=Net2Vec),size=10,label="\u25D7",family="Arial Unicode MS")+theme_classic()+xlim(0,12)+ylim(-.3,.3)

# yeo17

agF_y17<-ggplot(ageAligneddf_stringent,aes(x=tmdifvec,y=noAge_EFEfVec,color=Net1Vec17)) + geom_text(size=10,label="\u25D6",family="Arial Unicode MS") + xlab('Transmodality Difference') + ylab('EF Partial Correlation') + ggtitle('Pairwise Between-Network Age-aligned EF-FC RelationsL: Yeo17')+scale_color_manual(values=y17colors,limits=y17vec)+geom_text(aes(x=tmdifvec,y=noAge_EFEfVec,color=Net2Vec17),size=10,label="\u25D7",family="Arial Unicode MS")+theme_classic()+xlim(0,12)+ylim(-.3,.3)

agM_y17<-ggplot(ageMitigateddf_stringent,aes(x=tmdifvec,y=noAge_EFEfVec,color=Net1Vec17)) + geom_text(size=10,label="\u25D6",family="Arial Unicode MS") + xlab('Transmodality Difference') + ylab('EF Partial Correlation') + ggtitle('Pairwise Between-Network Age-mitigated EF-FC Relations: Yeo17')+scale_color_manual(values=y17colors,limits=y17vec)+geom_text(aes(x=tmdifvec,y=noAge_EFEfVec,color=Net2Vec17),size=10,label="\u25D7",family="Arial Unicode MS")+theme_classic()+xlim(0,12)+ylim(-.3,.3)
