...
```r
ggplot(bwdf,aes(scalesvec,AB_Est,color=domnetvec17)) + xlab("# of Networks") + ylab('AB Path Coefficient') +theme_classic(base_size = 30) +guides(alpha=FALSE,color=guide_legend(title=" Yeo17 Overlap"))+theme(legend.position='top')+geom_smooth(data=subset(bwdf,domnetvec17=='Somatomotor A'),method='gam',formula = y~s(x,k=3))+geom_smooth(data=subset(bwdf,domnetvec17=='DM_B'),method='gam',formula = y~s(x,k=3))+geom_point(data=subset(bwdf,domnetvec17=='Somatomotor A'),size=4)+geom_point(data=subset(bwdf,domnetvec17=='DM_B'),size=4)+theme(legend.position='top',legend.text = element_text(size=22),legend.title = element_text(size=26))+scale_x_continuous(breaks=c(4,10,16,22,28))+scale_color_manual(values=c('#bc0943','#4183a8'),labels=c('Default Mode B','Somatomotor A'))
```
